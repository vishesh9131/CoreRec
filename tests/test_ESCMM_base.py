...
from corerec.engines.unionizedFilterEngine.nn_base.ESCMM_base import ESCMM_base


class TestESCMM:
    @pytest.fixture
    def sample_data(self):
        # Generate sample data for ESCMM model
        data = []
        for i in range(100):
            data.append(
                {
                    ...
                    "conversion_label": float(np.random.randint(0, 2))
    ...
    def test_initialization(self, model):
        assert model.name == "ESCMM"
        assert model.embed_dim == 16
        assert model.num_filters == 32
        assert model.kernel_sizes == [2, 3]
    ...
    def test_recommend(self, model, sample_data):
        ...
        for task in ["ctr", "cvr", "ctcvr"]:
            recommendations = model.recommend(
                user_features, item_pool=item_pool, top_n=3, task=task
            )
    ...
    def test_save_load(self, model, sample_data, tmp_path):
        model.fit(sample_data)
        save_path = tmp_path / "escmm_model.pt"
        model.save(str(save_path))

        loaded_model = ESCMM_base.load(str(save_path))
