...
class HYBRID_COLLABORATIVE:
    ...
    def _compute_similarity(self, matrix: np.ndarray, metric: str = "cosine") -> np.ndarray:
        ...
        if metric == "cosine":
            ...
            union = (
                np.sum(binary_matrix, axis=1, keepdims=True)
    ...
    def _collaborative_filtering_predict(self, user_id: int, item_id: int) -> float:
    ...
    def _content_based_predict(self, user_id: int, item_id: int) -> float:
        ...
        # cosine similarity
        similarities = cosine_similarity(target_features.reshape(1, -1), rated_features).flatten()
    ...
    def _weighted_hybrid_predict(self, user_id: int, item_id: int) -> float:
        ...
        prediction = self.cf_weight * cf_pred + self.content_weight * content_pred

        return prediction

    def _switching_hybrid_predict(self, user_id: int, item_id: int) -> float:
    ...
    def _mixed_hybrid_predict(self, user_id: int, item_id: int) -> float:
        ...
        prediction = 0.4 * item_cf_pred + 0.3 * user_cf_pred + 0.3 * content_pred
        ...
        if compute_similarities:
            ...
            if self.cf_method in ["item_based", "mixed"]:
                ...
                self.item_similarity = self._compute_similarity(
                    user_item_matrix.T, self.similarity_metric
                )
            ...
            if self.cf_method in ["user_based", "mixed"]:
                ...
                self.user_similarity = self._compute_similarity(
                    user_item_matrix, self.similarity_metric
                )
            ...
            if content_features is not None:
                ...
                self.content_similarity = self._compute_similarity(content_features, "cosine")

        self.is_trained = True

        if verbose:
            print(
                f"Training complete! Model ready for {self.n_users} users and {self.n_items} items."
            )
    ...
    def recommend(
        self, user_id: int, top_n: int = 10, exclude_known: bool = True, return_scores: bool = True
    ...
    def evaluate_components(self, user_id: int, item_id: int) -> Dict[str, float]:
        """
    ...
    def get_similar_items(
        self, item_id: int, top_n: int = 10, use_content: bool = False
