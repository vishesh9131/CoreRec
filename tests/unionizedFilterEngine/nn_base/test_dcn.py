...
def test_dcn():
    ...
    print(
        f"Dataset: {num_interactions} samples, {num_users} users, {num_items} items")
    print(
        f"Positive samples: {
            sum(ratings)}/{
            len(ratings)} ({
                sum(ratings) / len(ratings) * 100:.1f}%)")
    ...
    try:
        ...
        try:
            ...
            print(
                f"Top 5 recommendations for user {test_user}: {recommendations}")
