...
class DeepFMModel(nn.Module):
    ...
    def __init__(
        self, field_dims: List[int], embed_dim: int, mlp_dims: List[int], dropout: float = 0.0
    ):
        """
...
class DeepFM_base(BaseRecommender):
    """
    ...
    def _extract_features(self, interactions: List[Tuple]) -> None:
        ...
        if not self.feature_names:
            if (
                len(interactions) > 0
                and len(interactions[0]) > 2
                and isinstance(interactions[0][2], dict)
            ):
        ...
        for feature in self.feature_names:
            ...
            if all(isinstance(v, (str, bool)) for v in values) or all(
                isinstance(v, (int, float)) and v == int(v) for v in values
            ):
                # Categorical feature
    ...
    def _build_model(self) -> None:
        ...
        self.model = DeepFMModel(field_dims, self.embed_dim, self.mlp_dims, self.dropout).to(
            self.device
        )
            ...
            for feature in self.feature_names:
                if feature in self.categorical_features:
                    ...
                    if feature in features and features[feature] is not None:
                        ...
                        normalized = (
                            float(features[feature]) - self.numerical_means[feature]
                        ) / self.numerical_stds[feature]
    ...
    def _prepare_batch(
        self, interactions: List[Tuple], add_negative_samples: bool = True
            ...
            for user_id, _, features in interactions:
                if user_id in self.user_map:
                    ...
                    for _ in range(1):  # One negative per positive
                        ...
                        user_idx = self.user_map[user_id]
                        if any(
                            u_id == user_id and i_id == neg_item_id
                            for u_id, i_id, _ in interactions
                        ):
                            continue
    ...
    def predict(
        self, user_id: Any, item_id: Any, features: Optional[Dict[str, Any]] = None
    ) -> float:
        """
    ...
    def export_feature_importance(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            importance["item"] = float(
                np.abs(
                    linear_weights[len(self.user_map) : len(self.user_map) + len(self.item_map)]
                ).mean()
            )
    ...
    def update_incremental(
        ...
        if new_interactions:
            ...
            if hasattr(self, "model") and self.model is not None:
                ...
                old_num_users = (
                    len(self.user_map) - len(new_users) if new_users else len(self.user_map)
                )
                old_num_items = (
                    len(self.item_map) - len(new_items) if new_items else len(self.item_map)
                )
