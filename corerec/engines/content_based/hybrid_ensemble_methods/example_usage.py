...
def example_attention_mechanisms():
    ...
    attention_model = ATTENTION_MECHANISMS(
        num_heads=4, embedding_dim=64, dropout_rate=0.1, random_state=42
    )

    # train the model
    print("\nTraining attention-based model...")
    attention_model.train(
        user_item_matrix=user_item_matrix, epochs=5, learning_rate=0.01, verbose=True
    )
    ...
    recommended_items, scores = attention_model.recommend(
        user_id=user_id, top_n=5, exclude_known=True, known_items=known_items
    )
...
def example_ensemble_methods():
    ...
    ensemble = ENSEMBLE_METHODS(
        ensemble_strategy="weighted_average", normalize_scores=True, random_state=42
    )
    ...
    recommended_items, scores = ensemble.recommend(user_id=user_id, top_n=5, return_scores=True)
...
def example_hybrid_collaborative():
    ...
    recommended_items, scores = hybrid_model.recommend(
        user_id=user_id, top_n=5, exclude_known=True, return_scores=True
    )
    ...
    similar_items_cf, similarities_cf = hybrid_model.get_similar_items(
        item_id=target_item, top_n=5, use_content=False
    )
    ...
    similar_items_content, similarities_content = hybrid_model.get_similar_items(
        item_id=target_item, top_n=5, use_content=True
    )
...
def example_comparison():
    ...
    user_item_matrix, item_features = create_sample_data(n_users=50, n_items=30, density=0.15)
    ...
    attn_items, attn_scores = attention_model.recommend(
        user_id, top_n=top_n, known_items=np.where(user_item_matrix[user_id, :] > 0)[0]
    )
    ...
    print("\n2. Hybrid Collaborative:")
    hybrid_model = HYBRID_COLLABORATIVE(
        hybrid_strategy="weighted", cf_weight=0.6, content_weight=0.4, random_state=42
    )
