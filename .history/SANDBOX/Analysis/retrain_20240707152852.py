...
# DeepWalk model
from torch_geometric.nn import DeepWalk
...
models_to_benchmark = {
    ...
    "GCN": GCN(),
    "GraphSAGE": GraphSAGE(),
    "TransE": TransE(num_entities=num_nodes, num_relations=10, embedding_dim=16),
    "TransR": TransR(num_entities=num_nodes, num_relations=10, embedding_dim=16),
    "DistMult": DistMult(num_entities=num_nodes, num_relations=10, embedding_dim=16),
    "ComplEx": ComplEx(num_entities=num_nodes, num_relations=10, embedding_dim=16),
    "HAN": HAN(in_channels=16, out_channels=num_nodes, num_meta_paths=3),
    ...
    "SR-GNN": SRGNN(num_nodes=num_nodes, embedding_dim=16),
    "DeepWalk": DeepWalkModel(edge_index=edge_index, embedding_dim=16, walk_length=5, context_size=3, walks_per_node=2, num_nodes=num_nodes),
        ...
        for epoch in range(5):  # Further reduce the number of epochs
            ...
            for batch in loader:
                ...
                if psutil.virtual_memory().percent > 80:
                    raise MemoryError("Memory usage exceeded 80%")
