...
class LightGCN(BaseRecommender):
    ...
    def fit(self, interaction_matrix: csr_matrix, user_ids: List[int], item_ids: List[int]) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            # Create batches
            n_batches = len(users) // self.batch_size + (
                1 if len(users) % self.batch_size != 0 else 0
            )
    ...
    def save_model(self, filepath: str) -> None:
        ...
        model_data = {
            ...
            "user_embedding": self.user_embedding.detach().cpu().numpy()
            if self.user_embedding is not None
            else None,
            "item_embedding": self.item_embedding.detach().cpu().numpy()
...
class LightGCNModel(nn.Module):
    ...
    def __init__(
        self, n_users: int, n_items: int, n_factors: int, n_layers: int, dropout: float = 0.0
    ):
        """
        Initialize LightGCN model
