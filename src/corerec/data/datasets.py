...
class RecommendationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.user_features is not None:
            sample["user_features"] = self.user_features.get(
                user_id, torch.zeros(next(iter(self.user_features.values())).shape)
            )

        if self.item_features is not None:
            sample["item_features"] = self.item_features.get(
                item_id, torch.zeros(next(iter(self.item_features.values())).shape)
            )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "next_item":
            ...
            input_seq = torch.tensor(
                [self.item_map.get(item, 0) for item in input_seq], dtype=torch.long
            )
            target = torch.tensor(self.item_map.get(target, 0), dtype=torch.long)

            return {"user_id": user_id, "input_seq": input_seq, "target": target}
            ...
            masked_tokens = torch.tensor(
                [self.item_map.get(item, 0) for item in masked_tokens], dtype=torch.long
            )
            ...
            return {"user_id": user_id, "tokens": tokens, "attention_mask": attention_mask}
