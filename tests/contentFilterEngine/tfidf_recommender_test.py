...
def load_tfidf_module():
    # Compute absolute path to the module file to avoid importing
...
class TestTFIDFRecommender(unittest.TestCase):
    ...
    def test_similarity_self_highest(self):
        sim = self.rec.similarity_matrix
        # diagonal should be 1 for non-zero rows (cosine similarity), 0 for
        # zero vector row
    ...
    def test_recommend_similarity_order(self):
        # For item A (index 0), B (index 1) should appear among top similar
        # items
