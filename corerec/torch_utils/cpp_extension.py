...
def _find_rocm_home() -> Optional[str]:
    ...
    if rocm_home and torch.version.hip is None:
        print(
            f"No ROCm runtime is found, using ROCM_HOME='{rocm_home}'",
            file=sys.stderr)
    return rocm_home
...
def _join_rocm_home(*paths) -> str:
    ...
    if ROCM_HOME is None:
        ...
        raise OSError(
            "Building PyTorch extensions using "
...
IS_HIP_EXTENSION = True if (
    (ROCM_HOME is not None) and (
...
def check_compiler_ok_for_platform(compiler: str) -> bool:
    ...
    if compiler_path is None:
        return False
    # Use os.path.realpath to resolve any symlinks, in particular from 'c++'
    # to e.g. 'g++'.
    compiler_path = os.path.realpath(compiler_path)
    # Check the compiler name
    if any(name in compiler_path for name in _accepted_compilers_for_platform()):
        return True
    # If compiler wrapper is used try to infer the actual compiler by invoking
    # it with -v flag
    env = os.environ.copy()
    ...
    if IS_LINUX:
        ...
        if os.path.basename(
                compiler_path) == "c++" and "gcc version" in version_string:
            return True
        return any(
            name in compiler_path for name in _accepted_compilers_for_platform())
    if IS_MACOS:
...
def get_compiler_abi_compatibility_and_version(
    ...
    # First check if the compiler is one of the expected ones for the
    # particular platform.
    ...
    if IS_MACOS:
        # There is no particular minimum version we need for clang, so we're
        # good here.
        return (True, TorchVersion("0.0.0"))
    try:
        if IS_LINUX:
            ...
            version = versionstr.decode(
                *SUBPROCESS_DECODE_ARGS).strip().split(".")
        else:
            ...
            match = re.search(
                ...
                compiler_info.decode(
                    *SUBPROCESS_DECODE_ARGS).strip())
            version = [
                "0", "0", "0"] if match is None else list(
                match.groups())
    except Exception:
...
def _check_cuda_version(
        compiler_name: str,
        compiler_version: TorchVersion) -> None:
    if not CUDA_HOME:
        raise RuntimeError(CUDA_NOT_FOUND_MESSAGE)

    nvcc = os.path.join(CUDA_HOME, "bin", "nvcc")
    cuda_version_str = (subprocess.check_output(
        [nvcc, "--version"]).strip().decode(*SUBPROCESS_DECODE_ARGS))
    ...
    if cuda_ver != torch_cuda_version:
        ...
        if cuda_ver.major != torch_cuda_version.major:
            raise RuntimeError(
                CUDA_MISMATCH_MESSAGE.format(
                    cuda_str_version,
                    torch.version.cuda))
        warnings.warn(
            CUDA_MISMATCH_WARN.format(
                cuda_str_version,
                torch.version.cuda))

    if not (
        sys.platform.startswith("linux")
        and os.environ.get("TORCH_DONT_CHECK_COMPILER_ABI") not in ["ON", "1", "YES", "TRUE", "Y"]
    ...
    if cuda_str_version not in cuda_compiler_bounds:
        ...
        max_excl_compiler_version_str = ".".join(
            map(str, max_excl_compiler_version))
...
class BuildExtension(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        for extension in self.extensions:
            ...
            self._add_compile_flag(
                extension, "-DTORCH_API_INCLUDE_EXTENSION_H")
            # See note [Pybind11 ABI constants]
            for name in ["COMPILER_TYPE", "STDLIB", "BUILD_ABI"]:
                val = getattr(torch._C, f"_PYBIND11_{name}")
                if val is not None and not IS_WINDOWS:
                    self._add_compile_flag(
                        extension, f'-DPYBIND11_{name}="{val}"')
            self._define_torch_extension_name(extension)
            self._add_gnu_cpp_abi_flag(extension)

            if "nvcc_dlink" in extension.extra_compile_args:
                assert (
                    self.use_ninja), f"With dlink=True, ninja is required to build cuda extension {
                    extension.name}."
        ...
        def unix_cuda_flags(cflags):
            ...
            # overriding the option if the user explicitly passed it.
            _ccbin = os.getenv("CC")
            if _ccbin is not None and not any(flag.startswith(
                    ("-ccbin", "--compiler-bindir")) for flag in cflags):
        ...
        def unix_wrap_ninja_compile(
            ...
            common_cflags = self.compiler._get_cc_args(
                pp_opts, debug, extra_preargs)
            ...
            if with_cuda:
                ...
                if IS_HIP_EXTENSION:
                    cuda_post_cflags = cuda_post_cflags + \
                        _get_rocm_arch_flags(cuda_post_cflags)
            ...
            if isinstance(
                    extra_postargs,
                    dict) and "nvcc_dlink" in extra_postargs:
                cuda_dlink_post_cflags = unix_cuda_flags(
                    extra_postargs["nvcc_dlink"])
            else:
            ...
            _write_ninja_file_and_compile_objects(
                ...
                objects=objects,
                cflags=[
                    shlex.quote(f) for f in extra_cc_cflags +
                    common_cflags],
                post_cflags=[
                    shlex.quote(f) for f in post_cflags],
            ...
            def spawn(cmd):
                ...
                if len(src_list) >= 1 and len(obj_list) >= 1:
                    ...
                    if _is_cuda_file(src):
                        ...
                        cflags = win_cuda_flags(
                            cflags) + ["-std=c++17", "--use-local-env"]
                        for flag in COMMON_MSVC_FLAGS:
                            cflags = ["-Xcompiler", flag] + cflags
                        for ignore_warning in MSVC_IGNORE_CUDAFE_WARNINGS:
                            cflags = [
                                "-Xcudafe", "--diag_suppress=" + ignore_warning] + cflags
        ...
        def win_wrap_ninja_compile(
            ...
            # in setuptools.setup, and we convert the relative path to absolute
            # path here
            convert_to_absolute_paths_inplace(self.compiler.include_dirs)
            ...
            if isinstance(
                    extra_postargs,
                    dict) and "nvcc_dlink" in extra_postargs:
                cuda_dlink_post_cflags = win_cuda_flags(
                    extra_postargs["nvcc_dlink"])
            else:
                cuda_dlink_post_cflags = None
    ...
    def _add_gnu_cpp_abi_flag(self, extension):
        ...
        self._add_compile_flag(
            extension, "-D_GLIBCXX_USE_CXX11_ABI=" + str(int(torch._C._GLIBCXX_USE_CXX11_ABI))
        )
...
def _get_glibcxx_abi_build_flags():
    glibcxx_abi_cflags = ["-D_GLIBCXX_USE_CXX11_ABI=" +
...
def _check_and_build_extension_h_precompiler_headers(
        ...
        if not os.path.exists(path_dir):
            try:
                ...
                if exc.errno != errno.EEXIST:
                    raise RuntimeError(
                        f"Fail to create path {path_dir}") from exc
    ...
    def build_precompile_header(pch_cmd):
        try:
            subprocess.check_output(
                pch_cmd, shell=True, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(
                f"Compile PreCompile Header fail, command: {pch_cmd}") from e
    ...
    torch_include_dirs = [
        ...
        # torch/all.h
        "-I {}".format(os.path.join(lib_include, "torch",
                       "csrc", "api", "include")),
    ]
    ...
    if os.path.isfile(head_file_pch) is not True:
        ...
        b_same_sign = check_pch_signature_in_file(
            head_file_signature, pch_sign)
...
def load_inline(
    ...
    if functions is not None:
        ...
        if isinstance(functions, list):
            ...
            raise ValueError(
                f"Expected 'functions' to be a list or dict, but was {
                    type(functions)}")
                ...
                with GeneratedFileCleaner(keep_intermediates=keep_intermediates) as clean_ctx:
                    if IS_HIP_EXTENSION and (with_cuda or with_cudnn):
                        hipify_result = hipify_python.hipify(
                            ...
                            header_include_dirs=(
                                extra_include_paths if extra_include_paths is not None else []
                            ),
                            extra_files=[os.path.abspath(s) for s in sources],
                            ignores=[
                                _join_rocm_home("*"),
    ...
    if with_cuda:
        ...
        if IS_WINDOWS:
            ...
            if not os.path.exists(
                    _join_cuda_home(extra_lib_dir)) and os.path.exists(
                    _join_cuda_home("lib")):
...
def _get_cuda_arch_flags(cflags: Optional[List[str]] = None) -> List[str]:
    ...
    # If not given, determine what's best for the GPU / CUDA version that can
    # be found
    if not _arch_list:
        warnings.warn(
        ...
        # which could be of different types - therefore all archs for visible
...
def _get_build_directory(name: str, verbose: bool) -> str:
    ...
    if not os.path.exists(build_directory):
        if verbose:
            print(
                f"Creating extension directory {build_directory}...",
                file=sys.stderr)
...
def _write_ninja_file_to_build_library(
    ...
    if IS_WINDOWS:
        common_cflags += [
            f"-I{include}" for include in user_includes +
            system_includes]
    else:
        common_cflags += [
            f"-I{shlex.quote(include)}" for include in user_includes]
        common_cflags += [
    ...
    if IS_WINDOWS:
        cflags = common_cflags + COMMON_MSVC_FLAGS + \
            ["/std:c++17"] + extra_cflags
        cflags = _nt_quote_args(cflags)
    else:
        cflags = common_cflags + ["-fPIC", "-std=c++17"] + extra_cflags

    if with_cuda and IS_HIP_EXTENSION:
        cuda_flags = ["-DWITH_HIP"] + cflags + \
            COMMON_HIP_FLAGS + COMMON_HIPCC_FLAGS
        ...
        if IS_WINDOWS:
            ...
            for ignore_warning in MSVC_IGNORE_CUDAFE_WARNINGS:
                cuda_flags = ["-Xcudafe",
                              "--diag_suppress=" + ignore_warning] + cuda_flags
...
def _write_ninja_file(
    ...
    if with_cuda or cuda_dlink_post_cflags:
        if "PYTORCH_NVCC" in os.environ:
            # user can set nvcc compiler with ccache using the environment
            # variable here
    ...
    if with_cuda:
        ...
        # Nvcc flag `--generate-dependencies-with-compile` is not supported by
        # sccache, which may increase build time.
        if (torch.version.cuda is not None and os.getenv(
                "TORCH_EXTENSION_SKIP_NVCC_GEN_DEPENDENCIES", "0") != "1"):
