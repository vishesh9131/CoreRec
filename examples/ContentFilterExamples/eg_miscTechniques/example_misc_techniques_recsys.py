...
class MovieRecommender:
    def __init__(self):
        self.feature_selector = cr.FeatureSelection(k=50, method="chi2")
        self.noise_handler = cr.NoiseHandling(method="isolation_forest", contamination=0.1)
        self.cold_start_handler = cr.ColdStart(method="hybrid", n_neighbors=10)
    ...
    def preprocess_data(self):
        ...
        self.processed_features = self.feature_selector.fit_transform(
            clean_features, self.movies_df["genres"].str.split("|").apply(len)
        )
        ...
        self.cold_start_handler.fit(self.processed_features, interaction_matrix=interaction_matrix)
