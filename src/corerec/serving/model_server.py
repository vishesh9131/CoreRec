...
class ModelServer:
    ...
    def __init__(self, model, host: str = "0.0.0.0", port: int = 8000, enable_docs: bool = True):
        """
        ...
        async def recommend(request: RecommendationRequest):
            ...
            try:
                recs = self.model.recommend(
                    request.user_id, top_k=request.top_k, exclude_items=request.exclude_items
                )
                return {"user_id": request.user_id, "recommendations": recs, "top_k": request.top_k}
            except Exception as e:
        ...
        async def batch_recommend(request: BatchRecommendationRequest):
            ...
            try:
                if hasattr(self.model, "batch_recommend"):
                    ...
                    recs = {
                        uid: self.model.recommend(uid, request.top_k) for uid in request.user_ids
                    }
