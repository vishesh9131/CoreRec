...
# Loading the REES46 eCommerce dataset
data = pd.read_csv('src/SANDBOX/dataset/REES46/events.csv')
...
data['product_id'] = data['product_id'].map(item_id_map)
...
user_ids_batch = torch.randint(0, num_users, (batch_size,))
item_ids_batch = torch.randint(0, num_items, (batch_size,))
labels_batch = torch.randint(0, 2, (batch_size,)).float()

# Training step
loss = recommender.train_step(user_ids_batch, item_ids_batch, labels_batch)
