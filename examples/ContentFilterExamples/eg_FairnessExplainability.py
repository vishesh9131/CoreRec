...
context_recommender = cr.ContextAware(
    context_config_path="src/SANDBOX/dataset/ml-1m/context_config.json", item_features=item_features
)
item_recommender = cr.ItemProfiling()

# Initialize Fairness and Explainability Modules
explainable = cr.Explainable()
fairness_aware = cr.FairnessAware()
...
# Ensure Fairness
min_length = (
    min(len(v) for v in user_interactions.values()) if isinstance(user_interactions, dict) else None
)
user_interactions_df = (
    pd.DataFrame({k: v[:min_length] for k, v in user_interactions.items()})
    if min_length
    else pd.DataFrame(user_interactions)
)
