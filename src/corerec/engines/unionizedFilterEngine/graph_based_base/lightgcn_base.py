...
class LightGCNBase(BaseRecommender):
    ...
    def _build_model(self, n_users: int, n_items: int) -> tf.keras.Model:
        ...
        self.user_embeddings = tf.Variable(
            initializer([n_users, self.embedding_dim]), name="user_embeddings", dtype=tf.float32
        )
        self.item_embeddings = tf.Variable(
            initializer([n_items, self.embedding_dim]), name="item_embeddings", dtype=tf.float32
        )
    ...
    def _create_bpr_loss(self, users, pos_items, neg_items):
        ...
        # L2 regularization
        regularizer = (
            tf.nn.l2_loss(user_emb) + tf.nn.l2_loss(pos_item_emb) + tf.nn.l2_loss(neg_item_emb)
        )
        ...
        for epoch in range(self.epochs):
            ...
            for batch in range(n_batches):
                ...
                with tf.GradientTape() as tape:
                    ...
                    pos_item_emb = tf.nn.embedding_lookup(
                        self.item_final_embeddings, batch_pos_items
                    )
                    neg_item_emb = tf.nn.embedding_lookup(
                        self.item_final_embeddings, batch_neg_items
                    )
                    ...
                    regularizer = (
                        ...
                        + tf.nn.l2_loss(pos_item_emb)
                        + tf.nn.l2_loss(neg_item_emb)
                    )
                    reg_loss = self.regularization * regularizer
                ...
                # Compute gradients
                grads = tape.gradient(
                    total_batch_loss, [self.user_embeddings, self.item_embeddings]
                )
    ...
    def recommend(self, user_id: int, top_n: int = 10, exclude_seen: bool = True) -> List[int]:
        ...
        scores = (
            tf.matmul(tf.expand_dims(user_emb, 0), self.item_final_embeddings, transpose_b=True)
            .numpy()
            .flatten()
        )
    ...
    def load_model(cls, filepath: str) -> "LightGCNBase":
        ...
        # Restore embeddings
        instance.user_embeddings = tf.Variable(
            model_data["user_embeddings"], name="user_embeddings"
        )
        instance.item_embeddings = tf.Variable(
            model_data["item_embeddings"], name="item_embeddings"
        )
        instance.user_final_embeddings = tf.Variable(
            model_data["user_final_embeddings"], name="user_final_embeddings"
        )
        instance.item_final_embeddings = tf.Variable(
            model_data["item_final_embeddings"], name="item_final_embeddings"
        )
