...
class A2SVD(BaseRecommender):
    ...
    def _init_params(self, n_users: int, n_items: int) -> None:
        ...
        self.user_factors = self.create_tensor(
            np.random.normal(0, 0.1, (n_users, self.factors)), dtype="float32"
        )
        self.item_factors = self.create_tensor(
            np.random.normal(0, 0.1, (n_items, self.factors)), dtype="float32"
        )
    ...
    def _predict(self, user_idx: int, item_idx: int) -> float:
        ...
        if hasattr(xp, "dot"):  # NumPy-like API
            ...
            return (
                xp.dot(self.user_factors[user_idx], self.item_factors[item_idx]) + attention_score
            )
        else:  # PyTorch-like API
            attention_score = xp.sum(self.attention_weights[user_idx] * self.item_factors[item_idx])
            return (
                xp.sum(self.user_factors[user_idx] * self.item_factors[item_idx]) + attention_score
            )
        ...
        for iteration in range(self.iterations):
            ...
            for idx in indices:
                ...
                if hasattr(xp, "dot"):  # NumPy-like API
                    ...
                    # Update item factors
                    item_grad = (
                        error * (self.user_factors[user_idx] + self.attention_weights[user_idx])
                        - self.regularization * self.item_factors[item_idx]
                    )
                    self.item_factors[item_idx] += self.learning_rate * item_grad

                    # Update attention weights
                    attention_update = self.learning_rate * (
                        error * self.item_factors[item_idx]
                        - self.regularization * self.attention_weights[user_idx]
                    )
