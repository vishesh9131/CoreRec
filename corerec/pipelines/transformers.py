...
class CategoryEncoder(DataTransformer):
    ...
    def fit(self, data: pd.DataFrame) -> "CategoryEncoder":
        ...
        cols_to_encode = self.columns or data.select_dtypes(
...
class FeatureScaler(DataTransformer):
    ...
    def fit(self, data: pd.DataFrame) -> "FeatureScaler":
        ...
        cols_to_scale = self.columns or data.select_dtypes(
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, params in self.params.items():
            if col in data.columns:
                if self.method == "standard":
                    data[col] = (data[col] - params["mean"]) / \
                        (params["std"] + 1e-8)
                elif self.method == "minmax":
                    range_val = params["max"] - params["min"]
                    data[col] = (data[col] - params["min"]) / \
                        (range_val + 1e-8)
                elif self.method == "robust":
                    data[col] = (data[col] - params["median"]) / \
                        (params["iqr"] + 1e-8)
