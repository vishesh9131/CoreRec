...
class VMFBase(BaseRecommender):
    ...
    def _predict(self, user_idx: int, item_idx: int, sample: bool = False) -> float:
        ...
        if sample:
            ...
            user_factors = self._sample_factors(
                self.user_factors_mean[user_idx], self.user_factors_std[user_idx]
            )
            item_factors = self._sample_factors(
                self.item_factors_mean[item_idx], self.item_factors_std[item_idx]
            )
    ...
    def _kl_divergence(self, mean: np.ndarray, std: np.ndarray) -> float:
        ...
        return (
            0.5
            * np.sum(np.square(std) + np.square(mean - self.prior_mean) - 1 - 2 * np.log(std))
            / mean.size
        )
    ...
    def _sgd_update(self, user_idx: int, item_idx: int, rating: float, pred: float) -> None:
        ...
        grad_user_mean = (
            error * item_factors - self.regularization * self.user_factors_mean[user_idx]
        )
        grad_item_mean = (
            error * user_factors - self.regularization * self.item_factors_mean[item_idx]
        )
        ...
        kl_grad_user_std = (
            self.user_factors_std[user_idx] - 1.0 / self.user_factors_std[user_idx]
        ) / self.factors

        kl_grad_item_mean = (self.item_factors_mean[item_idx] - self.prior_mean) / self.factors
        kl_grad_item_std = (
            self.item_factors_std[item_idx] - 1.0 / self.item_factors_std[item_idx]
        ) / self.factors

        # Update factor means
        self.user_factors_mean[user_idx] += self.learning_rate * (
            grad_user_mean - kl_grad_user_mean
        )
        self.item_factors_mean[item_idx] += self.learning_rate * (
            grad_item_mean - kl_grad_item_mean
        )

        # Update factor standard deviations (ensure they remain positive)
        self.user_factors_std[user_idx] = np.maximum(
            0.01,
            self.user_factors_std[user_idx]
            + self.learning_rate * (grad_user_std - kl_grad_user_std),
        )
        self.item_factors_std[item_idx] = np.maximum(
            0.01,
            self.item_factors_std[item_idx]
            + self.learning_rate * (grad_item_std - kl_grad_item_std),
        )
