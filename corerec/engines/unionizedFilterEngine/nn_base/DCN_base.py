...
class DCN_base(BaseCorerec):
    ...
    def _prepare_batch(self, data: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, feat_name in enumerate(self.feature_names):
            if feat_name in data.columns:
                if isinstance(self.feature_mappings[feat_name], dict) and 'mean' in self.feature_mappings[feat_name]:
                    ...
                    features[:, i+2] = data[feat_name].map(self.feature_mappings[feat_name]).fillna(0).values
    ...
    def _prepare_data(self, data: List[Tuple] or pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(data, list):
            if len(data[0]) == 3:  # (user_id, item_id, rating)
                ...
                raise ValueError("Data format not recognized. Expected (user_id, item_id, rating[, timestamp])")
        else:
            df = data.copy()
        
        return df
    
    def fit(self, data: List[Tuple] or pd.DataFrame, validation_data: List[Tuple] or pd.DataFrame = None) -> 'DCN_base':
        """
        Train the DCN model.
        ...
        for epoch in range(self.num_epochs):
            ...
            with tqdm(total=len(indices), desc=f"Epoch {epoch+1}/{self.num_epochs}", disable=not self.verbose) as pbar:
    ...
    def predict(self, user_id: Any, item_id: Any, additional_features: Dict[str, Any] = None) -> float:
        """
    ...
    def recommend(self, user_id: Any, top_n: int = 10, additional_features: Dict[str, Any] = None, 
                 items_to_ignore: List[Any] = None) -> List[Tuple[Any, float]]:
        """
