...
class TestFLEN:
    ...
    def sample_data(self):
        # Generate sample data for FLEN model
        data = []
        for i in range(100):
            data.append({
    ...
    def test_fit(self, model, sample_data):
        ...
        assert hasattr(fitted_model, 'field_groups')
    ...
    def test_recommend(self, model, sample_data):
        ...
        recommendations = model.recommend(user_features, item_pool=item_pool, top_n=3)
        assert len(recommendations) == 3
        assert all(0 <= score <= 1 for _, score in recommendations)
    
    def test_save_load(self, model, sample_data, tmp_path):
        model.fit(sample_data)
        save_path = tmp_path / "flen_model.pt"
        model.save(str(save_path))
        
        loaded_model = FLEN_base.load(str(save_path))
