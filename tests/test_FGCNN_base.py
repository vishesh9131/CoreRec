...
class TestFGCNN:
    ...
    def sample_data(self):
        # Generate sample data for FGCNN model
        data = []
        for i in range(100):
            data.append({
    ...
    def test_initialization(self, model):
        assert model.name == "FGCNN"
        assert model.embed_dim == 16
        assert model.channels == [32]
        assert model.kernel_heights == [3]
    ...
    def test_save_load(self, model, sample_data, tmp_path):
        model.fit(sample_data)
        save_path = tmp_path / "fgcnn_model.pt"
        model.save(str(save_path))
        
        loaded_model = FGCNN_base.load(str(save_path))
