...
from torch.nn import TransformerEncoder, TransformerEncoderLayer, Linear, Module, ModuleList
...
class GraphDataset(Dataset):
    def __init__(self, file_path):
        self.adj_matrix, self.weight_matrix = self.parse_content_file(file_path)
    ...
    def parse_content_file(self, file_path):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                for neighbor in parts[-1].split(','):
                    if neighbor:
                        edges.append((node_id, int(neighbor)))
        ...
        for node_id, features in enumerate(node_features):
            for neighbor in edges:
                adj_matrix[node_id, neighbor[1]] = 1
                weight_matrix[node_id, neighbor[1]] = 1  # Placeholder for weights
        
        return torch.tensor(adj_matrix, dtype=torch.float32), torch.tensor(weight_matrix, dtype=torch.float32)
