...
# Import AITM using the top-level module pattern
    ...
    def __init__(self, filepath, encoding="latin1"):
        ...
        self.genre_to_idx = {
            ...
            for idx, genre in enumerate(
                sorted(set([g for sublist in self.genres for g in sublist]))
            )
        }
...
def main():
    ...
    # Try to find the movies.dat file in common locations
    import os
    ...
    if dataset_path is None:
        print("Warning: movies.dat not found. Creating synthetic dataset for demonstration.")
        # Create a simple synthetic dataset
        class SyntheticMoviesDataset(Dataset):
            def __init__(self, num_movies=100, num_genres=18):
                self.num_genres = num_genres
                self.encoded_genres = np.random.rand(num_movies, num_genres).astype(np.float32)
                self.genre_to_idx = {f"genre_{i}": i for i in range(num_genres)}
            
            def __len__(self):
                return len(self.encoded_genres)
        ...
        dataset = SyntheticMoviesDataset(num_movies=100, num_genres=input_dim)
    else:
        dataset = MoviesDataset(dataset_path, encoding="latin1")
        # Update input_dim and output_dim based on actual dataset
        if hasattr(dataset, 'genre_to_idx'):
            input_dim = len(dataset.genre_to_idx)
            output_dim = len(dataset.genre_to_idx)
    
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

    # Initialize AITM
    model = aitm.AITM(input_dim, feature_dim, output_dim)
    ...
    model.evaluate(dataloader, criterion)

    # Recommend genres for a specific movie (if using real dataset)
    if hasattr(dataset, 'movies') and len(dataset.movies) > 0:
        movie_title = "Mr. Smith Goes to Washington (1939)"
        recommend_genres_for_movie(movie_title, dataset, model)
    else:
        print("Synthetic dataset used - skipping movie recommendation example")
