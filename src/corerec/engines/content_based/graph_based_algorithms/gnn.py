...
class GNNRecommender(BaseCorerec):
    ...
    def _build_graph(self, interactions):
        ...
        # Add nodes
        user_nodes = [
            (self.user_mapping[user], {"type": "user", "original_id": user})
            for user in unique_users
        ]
        item_nodes = [
            (self.item_mapping[item], {"type": "item", "original_id": item})
    ...
    def _build_model(self):
        ...
        class GNNModel(nn.Module):
            def __init__(
                ...
                for i in range(num_layers):
                    self.layers.append(
                        GraphConvLayer(embedding_dim, embedding_dim, residual=(i > 0))
                    )
        ...
        self.model = model
        self.logger.info(
            f"Built GNN model with {self.num_layers} layers and {self.embedding_dim} dimensions"
        )
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, len(train_data), self.batch_size):
                ...
                neg_item_indices = torch.randint(
                    0, self.num_items, size=pos_item_indices.size()
                ).to(self.device)
                ...
                # Compute predictions
                pos_scores = self.model.predict(user_indices, pos_item_indices, user_emb, item_emb)
                neg_scores = self.model.predict(user_indices, neg_item_indices, user_emb, item_emb)
