    ...
    def __init__(self, num_fields: int, embed_dim: int, bilinear_type: str = "field_all"):
        ...
        if bilinear_type == "field_all":
            ...
            self.bilinear_W = nn.Parameter(
                torch.Tensor(num_fields, num_fields, embed_dim, embed_dim)
            )
        else:
            raise ValueError(
                "bilinear_type must be 'field_all', 'field_each', or 'field_interaction'"
            )
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        if self.bilinear_type == "field_all":
            ...
            for i in range(self.num_fields):
                ...
                bilinear_out[:, i, 0] = torch.bmm(
                    torch.bmm(vi, self.bilinear_W[i].unsqueeze(0).expand(batch_size, -1, -1)),
                    vi.transpose(1, 2),
                ).squeeze()
            return bilinear_out
            ...
            bilinear_out = torch.zeros(
                batch_size, self.num_fields, self.embed_dim, device=embeddings.device
            )
            for i in range(self.num_fields):
                for j in range(self.num_fields):
                    if i != j:  # Exclude self-interactions
                        ...
                        bilinear_out[:, i, :] += torch.einsum(
                            "bi,ij,bj->bi", vi, self.bilinear_W[i][j], vj
                        )
            return bilinear_out
...
class Fibinet_base(BaseRecommender):
    """
