...
def initialize_recommender():
    ...
    try:
        hybrid_engine, movies_df, ratings_df, movie_id_map, reverse_movie_id_map = load_saved_model()
...
def get_next_recommendations(user_id: int, page_size: int = 1):
    ...
    if user_id in st.session_state.user_feedback:
        exclude_items = list(st.session_state.user_feedback[user_id]['liked']) + list(st.session_state.user_feedback[user_id]['disliked'])
    ...
    if user_id in st.session_state.user_feedback:
        ...
        if liked_movies:
            ...
            for m_id in liked_movies:
                genres = st.session_state.movies_df.loc[st.session_state.movies_df['movie_id'] == m_id, 'genres'].values
                if len(genres) > 0:
            ...
            recommendations = sorted(
                recommendations,
                key=lambda x: len(set(st.session_state.movies_df.loc[st.session_state.movies_df['movie_id'] == x, 'genres'].values[0].split('|')) & liked_genres),
                reverse=True
            )
...
def retrain_model():
    ...
    updated_ratings_df = pd.concat([st.session_state.ratings_df, st.session_state.feedback], ignore_index=True)
    
    # Update session state
    st.session_state.ratings_df = updated_ratings_df
    st.session_state.feedback = pd.DataFrame(columns=['user_id', 'movie_id', 'rating'])  # Reset feedback
...
def main():
    ...
    if recommendations:
        ...
        movie = st.session_state.movies_df.loc[st.session_state.movies_df['movie_id'] == movie_id].iloc[0]
