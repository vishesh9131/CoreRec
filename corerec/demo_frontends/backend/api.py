...
async def get_recommendations(user_id: str, request: RecommendationRequest):
    ...
    if df.empty:
        raise HTTPException(status_code=404, detail=f"No data found for platform: {request.platform}")
    ...
    if liked_items:
        ...
        recommendations = get_popular_items(filtered_df, request.platform, request.num_recommendations)
...
def get_content_based_recommendations(df: pd.DataFrame, liked_items: set, full_df: pd.DataFrame, 
                                    platform: str, num_recs: int) -> List[Dict]:
    ...
    if platform == "spotify":
        ...
        if all(col in liked_df.columns for col in feature_cols):
            ...
            df["similarity_score"] = (
                (1 - df["quality_score_diff"]) * 0.5 +
                (1 - df["engagement_rate_diff"]) * 0.5
            )
        ...
        df["genre_boost"] = df["genre"].map(
            lambda x: preferred_genres.get(x, 0) / len(liked_items)
        ).fillna(0)
        
        df["similarity_score"] = (
            df["rating_similarity"] * 0.6 +
            df["genre_boost"] * 0.4
        )
