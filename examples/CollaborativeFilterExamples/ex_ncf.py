...
print(
    f"Dataset: {len(ratings_df):,} ratings, {ratings_df['user_id'].nunique():,} users, {ratings_df['movie_id'].nunique():,} movies"
)
...
print(
    f"Positive ratings: {sum(sample_df['rating'])} ({sum(sample_df['rating'])/len(sample_df)*100:.1f}%)"
)
...
recommendation_titles = [
    movies_df[movies_df["movie_id"] == item_id]["title"].iloc[0] for item_id in recommendation_items
]
print(
    "Recommendation diversity: ✓"
    if len(set([title.split("(")[-1].split(")")[0] for title in recommendation_titles])) > 1
    else "✗"
)
