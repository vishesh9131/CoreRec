...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        self, hidden_dim: int, num_heads: int, feed_forward_dim: int, dropout: float = 0.1
    ):
        """
...
class BehaviorSequenceTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.feature_embedding = FeatureEmbedding(
            feature_field_dims, hidden_dim // len(feature_field_dims)
        )
    ...
    def forward(
        self, seq_items: torch.Tensor, target_item: torch.Tensor, target_features: torch.Tensor
    ) -> torch.Tensor:
        """
...
class BST_base(BaseRecommender):
    ...
    def fit(self, interactions, user_ids=None, item_ids=None, item_features=None):
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            if self.verbose:
                pbar = tqdm(
                    total=len(train_loader), desc=f"Epoch {epoch+1}/{self.config['num_epochs']}"
                )
    ...
    def _create_dataset(self):
        ...
        def recommend(self, user_id, top_n=10, exclude_seen=True):
            ...
            with torch.no_grad():
                ...
                batch_size = 100
                item_batches = [
                    list(range(1, self.num_items + 1))[i : i + batch_size]
                ...
                for item_batch in item_batches:
                    ...
                    for item_idx in item_batch:
                            ...
                            if isinstance(item_features, dict):
                                ...
                                for name, value_map in self.feature_value_maps.items():
                                    feature_indices.append(
                                        value_map.get(item_features.get(name, 0), 0)
                                    )
                        ...
                        item_features_tensor = torch.tensor(item_features, dtype=torch.long).to(
                            self.device
                        )
        ...
        def update_incremental(
            self, new_interactions, new_user_ids=None, new_item_ids=None, new_item_features=None
        ):
            """
        ...
        def _update_user_sequences(self, interactions):
            ...
            for user_idx, item_idx, _ in timestamp_data:
                ...
                if (
                    not self.user_sequences[user_idx]
                    or self.user_sequences[user_idx][-1] != item_idx
                ):
