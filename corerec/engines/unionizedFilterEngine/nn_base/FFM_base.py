    ...
    def forward(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        for i in range(self.num_fields):
            for j in range(i+1, self.num_fields):
                ...
                v_i_j = field_aware_embeddings[i][:, j-1] if j > i else field_aware_embeddings[i][:, j]
                v_j_i = field_aware_embeddings[j][:, i] if i < j else field_aware_embeddings[j][:, i-1]
