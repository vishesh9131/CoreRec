...
class DeepCrossingModel(nn.Module):
    ...
    def __init__(
        ...
        self.embeddings = nn.ModuleList(
            [
                nn.Embedding(
                    1000, embedding_dim, padding_idx=0
                )  # Default max size, will be resized later
...
class DeepCrossing_base(BaseRecommender):
    """
    ...
    def _preprocess_data(self, interactions):
        ...
        self.user_map = {
            user_id: idx + 1 for idx, user_id in enumerate(self.user_ids)
        }  # 0 reserved for padding
        self.item_map = {
            item_id: idx + 1 for idx, item_id in enumerate(self.item_ids)
        }  # 0 reserved for padding
        ...
        for feature in self.feature_names:
            ...
            self.feature_mappings[feature] = {
                val: idx + 1 for idx, val in enumerate(values)
            }  # 0 reserved for padding
    ...
    def _train(self, X, y, X_val=None, y_val=None):
        ...
        dataset = torch.utils.data.TensorDataset(
            torch.LongTensor(X).to(self.device), torch.FloatTensor(y).unsqueeze(1).to(self.device)
        )
    ...
    def recommend(self, user_id, top_n=10, features=None, exclude_seen=True):
        ...
        seen_items = set()
        if exclude_seen:
            for i, (uid, iid, _) in enumerate(
                zip(self.user_ids, self.item_ids, self.feature_names)
            ):
                if uid == user_id:
                    seen_items.add(iid)
    ...
    def export_feature_importance(self):
        ...
        return {
            feature: 1.0 / (2 + len(self.feature_names))
            for feature in ["user", "item"] + self.feature_names
        }
    ...
    def update_incremental(
        self, new_interactions: List[Tuple], new_user_ids: List = None, new_item_ids: List = None
    ):
        """
