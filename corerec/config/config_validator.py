...
class ConfigValidator:
    ...
    def add_rule(self, key: str, expected_type: type, required: bool = False,
    ...
    def validate(self, config: Dict[str, Any]) -> bool:
        ...
        for key, rule in self.rules.items():
            ...
            if not isinstance(value, rule['type']):
                self.errors.append(
                    f"Key '{key}' has wrong type: expected {rule['type'].__name__}, got {type(value).__name__}"
                )
                continue
            
            # Check range for numbers
            if rule['min'] is not None and value < rule['min']:
                self.errors.append(f"Key '{key}' value {value} is below minimum {rule['min']}")
            
            if rule['max'] is not None and value > rule['max']:
                self.errors.append(f"Key '{key}' value {value} is above maximum {rule['max']}")
            
            # Check allowed values
            if rule['allowed'] is not None and value not in rule['allowed']:
                self.errors.append(f"Key '{key}' value {value} not in allowed values {rule['allowed']}")
