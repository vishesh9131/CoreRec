...
from corerec.engines.collaborative.nn_base.Bert4Rec_base import Bert4Rec_base
from corerec.engines.collaborative.nn_base.Bert4Rec_base import (
    BERT4RecModel,
    MultiHeadAttention,
    TransformerBlock,
    TokenEmbedding,
...
def patched_build_model(self):
    ...
    self.trainer = MockTrainer(model=self.model, config=self.config, device=self.device)
...
def patched_fit(self, interaction_matrix, user_ids, item_ids):
    ...
    # Create dataset
    dataset = MockDataset(sequences=self.user_sequences, max_seq_len=self.config["max_seq_len"])
...
class TestBERT4RecComponents(unittest.TestCase):
    ...
    def test_hook_manager(self):
        ...
        model = nn.Sequential(nn.Linear(10, 5, bias=True), nn.ReLU(), nn.Linear(5, 1, bias=True))
