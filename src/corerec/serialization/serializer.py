...
def deserialize(data: Union[Dict[str, Any], str, Path]) -> Any:
    ...
    if isinstance(data, (str, Path)):
        if os.path.exists(data):
            ...
            try:
                ...
                raise ValueError(f"Invalid input: not a file path or JSON string")
    ...
    if hasattr(klass, "from_dict"):
        ...
        constructor_args = {k: v for k, v in data.items() if not k.startswith("_")}
...
def save_to_file(obj: Any, file_path: str, format: str = "json"):
    ...
    if format == "json":
        ...
        raise ValueError(f"Unsupported format: {format}. Use 'json', 'yaml', or 'pickle'")
...
def load_from_file(file_path: str) -> Any:
    ...
    if file_ext == ".json":
        ...
        raise ValueError(f"Unsupported file format: {file_ext}. Use .json, .yaml, or .pkl")
