...
class _Column:
    def __init__(
        ...
        self._template = (
            f"{{:>{length}.{decimal_digits}f}}{{:>{7 if self._highlight_warnings else 0}}}"
        )
    ...
    def num_to_str(
        ...
        return self._template.format(
            ...
            f" (! {
                spread *
                100:.0f}%)" if self._highlight_warnings and spread is not None else "",
        )
...
class _Row:
    ...
    def finalize_column_strings(self, column_strings, col_widths):
        ...
        if self._colorize == Colorize.ROWWISE:
            ...
            best_values = [
                optional_min(
                    r.median for r in column.get_results_for(self._row_group) if r is not None
                )
        ...
        for col_str, width, result, best_value in zip(
            ...
            if self._colorize != Colorize.NONE and result is not None and best_value is not None:
                col_str = self.color_segment(
                    col_str, result.median, best_value)
...
class Table:
    def __init__(
        ...
        self.row_keys = common.ordered_unique(
            [self.row_fn(i) for i in results])
        self.row_keys.sort(
            key=operator.itemgetter(
                slice(2)))  # preserve stmt order
        self.column_keys = common.ordered_unique(
            [self.col_fn(i) for i in results])
    ...
    def populate_rows_and_columns(
        ...
        for i in range(len(self.column_keys)):
            grouped_results = [tuple(row[i] for row in g)
...
class Compare:
    ...
    def extend_results(self, results):
        ...
        for r in results:
            if not isinstance(r, common.Measurement):
                raise ValueError(
                    "Expected an instance of `Measurement`, " f"got {
                        type(r)} instead.")
    ...
    def _group_by_label(self, results: List[common.Measurement]):
        grouped_results: DefaultDict[str,
                                     List[common.Measurement]] = collections.defaultdict(list)
        for r in results:
