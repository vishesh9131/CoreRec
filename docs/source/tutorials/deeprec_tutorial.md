...
DeepRec uses a sophisticated architecture for recommendation tasks.
...
```python
from corerec.sandbox.collaborative_full.nn_base.DeepRec_base import DeepRec_base as DeepRec
...
```python
# Save model
model.save('artifacts/deeprec_model')

# Load model
loaded = DeepRec_base.load('artifacts/deeprec_model')
