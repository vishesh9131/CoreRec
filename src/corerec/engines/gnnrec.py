...
class GNNRec(BaseCorerec):
    ...
    def _build_model(self, num_users: int, num_items: int):
        ...
        return GNNModel(
            num_users, num_items, self.embedding_dim, self.num_gnn_layers, self.dropout
        ).to(self.device)
    ...
    def fit(
        ...
        adj_matrix = (
            torch.sparse.FloatTensor(indices, values, (adj_size, adj_size))
            .to_dense()
            .to(self.device)
        )
