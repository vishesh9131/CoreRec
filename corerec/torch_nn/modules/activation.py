...
class Threshold(Module):
    ...
    def __init__(
        ...
        # TODO: check in THNN (if inplace == True, then assert value <=
        # threshold)
...
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        if not self._qkv_same_embed_dim:
            self.q_proj_weight = Parameter(torch.empty(
                (embed_dim, embed_dim), **factory_kwargs))
            self.k_proj_weight = Parameter(torch.empty(
                (embed_dim, self.kdim), **factory_kwargs))
            self.v_proj_weight = Parameter(torch.empty(
    ...
    def forward(
        ...
        if not is_fastpath_enabled:
            ...
            why_not_fast_path = (
                f"input not batched; expected query.dim() of 3 but got {
                    query.dim()}")
            ...
            why_not_fast_path = f"dtypes of query ({
                query.dtype}) and self.in_proj_bias ({
                self.in_proj_bias.dtype}) don't match"
            ...
            # error message.
            why_not_fast_path = f"dtypes of query ({
                query.dtype}) and self.in_proj_weight ({
                self.in_proj_weight.dtype}) don't match"
        elif self.training:
        ...
        if not why_not_fast_path:
            ...
            if torch.overrides.has_torch_function(tensor_args):
                ...
                why_not_fast_path = (
                    "some Tensor argument's device is neither one of " f"cpu, cuda or {
                        torch.utils.backend_registration._privateuse1_backend_name}")
...
class Softmax2d(Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if input.dim() not in (3, 4):
            raise ValueError(
                f"Softmax2d: expected input to be 3D or 4D, got {
                    input.dim()}D instead")
