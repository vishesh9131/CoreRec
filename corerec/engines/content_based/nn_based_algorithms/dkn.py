...
class DKN(nn.Module):
    def __init__(
        ...
        self.text_convs = nn.ModuleList(
            [
                nn.Conv1d(in_channels=embedding_dim, out_channels=text_num_filters, kernel_size=k)
        ...
        self.entity_embedding = nn.Embedding(
            num_embeddings=knowledge_graph_size, embedding_dim=entity_embedding_dim, padding_idx=0
        )
        ...
        if self.verbose:
            logger.info(
                f"Initializing MultiheadAttention with embed_dim={embed_dim} and num_heads={num_heads}"
            )
    ...
    def forward(self, text, entities):
        ...
        # Text CNN Encoder
        text_conv_out = [
            F.relu(conv(embedded_text)) for conv in self.text_convs
        ]  # List of (batch_size, num_filters, L_out)
        text_pooled = [
            F.max_pool1d(conv, conv.size(2)).squeeze(2) for conv in text_conv_out
        ...
        text_features, _ = self.attention(
        ...
        # Fusion Layer
        fused = torch.cat(
            [text_features, entity_features], dim=1
        )  # (batch_size, embed_dim + entity_embedding_dim)
