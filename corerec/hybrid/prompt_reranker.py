...
class OpenAIClient(LLMClient):
    ...
    async def generate_async(self, prompt: str, **kwargs) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a recommendation assistant. Output your choice directly with no explanation or additional content.",
            },
            {"role": "user", "content": prompt},
        ]

        # Create request data
        data = {"messages": messages, **params}
...
class AnthropicClient(LLMClient):
    ...
    async def generate_async(self, prompt: str, **kwargs) -> str:
        ...
        # Create request data
        data = {"messages": [{"role": "user", "content": prompt}], "system": system, **params}
...
class PromptReranker(BaseModel):
    ...
    def forward(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            items_str = "\n".join(
                [f"{item_id}: {item['description']}" for item_id, item in zip(item_ids, items)]
            )

            # Format prompt
            prompt = self.prompt_template.format(
                user_preferences=user_prefs, context=context, items=items_str
            )
    ...
    async def rerank_async(
        self, user_preferences: str, context: str, items: List[Dict[str, Any]], item_ids: List[str]
        ...
        # Format prompt
        prompt = self.prompt_template.format(
            user_preferences=user_preferences, context=context, items=items_str
        )
    ...
    def rerank(
        ...
        try:
            result = loop.run_until_complete(
                self.rerank_async(user_preferences, context, items, item_ids)
            )
        finally:
            loop.close()
        return result
    ...
    async def batch_rerank_async(
        ...
        for i in range(len(user_preferences_list)):
            task = self.rerank_async(
                user_preferences_list[i], context_list[i], items_list[i], item_ids_list[i]
            )
            tasks.append(task)
    ...
    def batch_rerank(
        ...
        try:
            result = loop.run_until_complete(
                self.batch_rerank_async(
                    user_preferences_list, context_list, items_list, item_ids_list
                )
            )
        finally:
            loop.close()
