...
class FormatMaster:
    ...
    def detect(self, data, config: dict = None, custom_preprocess: callable = None) -> pd.DataFrame:
        ...
        try:
            if isinstance(data, str):
                ...
                if data_path:
                    ...
                    if data_path.endswith(".csv"):
                        ...
                        df = dd.read_csv(
                            data_path, blocksize=config.get("chunk_size", "64MB"), dtype=dtype_spec
                        )
                        is_dask = True
                    else:
            ...
            logging.info(
                f"Preprocessed data has {preprocessed_df.shape[0]} rows and {preprocessed_df.shape[1]} columns"
            )
