...
def optimize_for_mobile(
    ...
    if not isinstance(script_module, torch.jit.ScriptModule):
        raise TypeError(
            f"Got {
                type(script_module)}, but ScriptModule is expected.")
    ...
    preserved_methods_str: List[str] = [
        str(method) for method in preserved_methods]

    bundled_inputs_attributes = _get_bundled_inputs_preserved_attributes(
        script_module, preserved_methods_str
    )
    if all(hasattr(script_module, method)
           for method in bundled_inputs_attributes):
    ...
    if non_exist_methods:
        raise AttributeError(
            f"The following methods to preserve do not exist in script_module: {
                ', '.join(non_exist_methods)}")
    ...
    if backend == "cpu":
        ...
        raise TypeError(
            "Unknown backend, must be one of 'CPU', 'Vulkan' or 'Metal'")
...
def _get_bundled_inputs_preserved_attributes(
    ...
    # Bundled inputs in module after the change that introduced bundled inputs
