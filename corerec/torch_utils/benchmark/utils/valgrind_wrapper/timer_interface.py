...
class FunctionCounts:
    ...
    def __getitem__(self, item: Any) -> Union[FunctionCount, "FunctionCounts"]:
        data: Union[FunctionCount,
                    Tuple[FunctionCount, ...]] = self._data[item]
        return (
            FunctionCounts(
                cast(Tuple[FunctionCount, ...], data), self.inclusive, truncate_rows=False
            )
    ...
    def __mul__(self, other: Union[int, float]) -> "FunctionCounts":
        return self._from_dict({fn: int(c * other)
                               for c, fn in self._data}, self.inclusive)
    ...
    def denoise(self) -> "FunctionCounts":
        ...
        return self.filter(
            lambda fn: "dictobject.c:lookdict_unicode" not in fn)
    ...
    @staticmethod
    def _from_dict(counts: Dict[str, int],
...
""".strip()
    ...
    def delta(
        ...
        exclusive basis.
        """
        return self.stats(inclusive=inclusive) - \
            other.stats(inclusive=inclusive)
...
class CopyIfCallgrind:
    ...
    def __init__(self, value: Any, *, setup: Optional[str] = None):
        for method, supported_types in _GLOBALS_ALLOWED_TYPES.items():
            ...
            supported_str = "\n".join([getattr(t, "__name__", repr(
                t)) for t in it.chain(_GLOBALS_ALLOWED_TYPES.values())])
    ...
    def unwrap_all(globals: Dict[str, Any]) -> Dict[str, Any]:
        return {k: (v.value if isinstance(v, CopyIfCallgrind) else v)
        ...
        for name, wrapped_value in self._globals.items():
            ...
            if wrapped_value.serialization == Serialization.PICKLE:
                ...
                raise NotImplementedError(
                    f"Unknown serialization method: {
                        wrapped_value.serialization}")
...
class _ValgrindWrapper:
    def __init__(self) -> None:
        ...
        if all(hasattr(torch._C, symbol) for symbol in valgrind_symbols):
            ...
            assert all(hasattr(self._bindings_module, symbol)
                       for symbol in valgrind_symbols)
        ...
        build_search = re.search(
            "BUILD_TYPE=(.+),",
            torch.__config__.show())  # type: ignore[no-untyped-call]
    ...
    def _validate(self) -> None:
        ...
        missing_cmds = [
            cmd for cmd,
            available in self._commands_available.items() if not available]
        if missing_cmds:
        ...
        try:
            if is_python:
                if self._bindings_module is not None:
                    shutil.copy(
                        self._bindings_module.__file__, os.path.join(
                            working_dir, os.path.split(
                                self._bindings_module.__file__)[1]), )
            ...
            if valgrind_invocation.returncode:
                ...
                raise OSError(
                    f"Failed to collect callgrind profile:\n{error_report}")
            ...
            def parse_output(fpath: str, inclusive: bool) -> FunctionCounts:
                ...
                for l in annotate_invocation_output.splitlines(keepends=False):
                    if scan_state == ScanState.SCANNING_FOR_TOTAL:
                        ...
                        if total_match:
                            program_totals = int(total_match.groups()[
                                                 0].replace(",", ""))
            ...
            def read_results(
                    i: int) -> Tuple[FunctionCounts, FunctionCounts, Optional[str]]:
