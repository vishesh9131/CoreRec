    ...
    if isinstance(data, torch.Tensor):
        ...
        try:
            if isinstance(data, collections.abc.MutableMapping):
                ...
                clone.update({k: pin_memory(sample, device)
                             for k, sample in data.items()})
                return clone
            else:
                # type: ignore[call-arg]
                return type(data)({k: pin_memory(sample, device)
                                   for k, sample in data.items()})
        except TypeError:
            # The mapping type may not support `copy()` / `update(mapping)`
            # or `__init__(iterable)`.
            return {k: pin_memory(sample, device)
                    for k, sample in data.items()}
        ...
        try:
            if isinstance(data, collections.abc.MutableSequence):
                ...
                return clone
            return type(data)([pin_memory(sample, device)
                               for sample in data])  # type: ignore[call-arg]
        except TypeError:
