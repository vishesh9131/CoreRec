...
def parse_args():
    ...
    parser.add_argument(
        "--world_size", type=int, default=None, help="World size for distributed training"
    )
    parser.add_argument(
        "--master_addr",
        type=str,
    ...
    parser.add_argument(
        "--master_port", type=str, default="12355", help="Master port for distributed training"
    )
...
def create_data_loader(
    config: Dict[str, Any], args, is_training: bool = True
) -> StreamingDataLoader:
...
def create_model(config: Dict[str, Any], args) -> MonolithModel:
    ...
    # Create model
    model = MonolithModel(name=model_config.get("name", "monolith_model"), config=model_config)
    ...
    if args.distributed and train_config.get("use_zero", False):
        ...
        if optimizer_name == "adagrad":
            optimizer = ZeroRedundancyOptimizer(
                param_groups, optimizer_class=torch.optim.Adagrad, lr=lr
            )
        else:  # Default to Adam
            optimizer = ZeroRedundancyOptimizer(
                param_groups, optimizer_class=torch.optim.Adam, lr=lr
            )
    else:
        ...
        if optimizer_name == "adagrad":
            ...
            optimizer = torch.optim.SGD(
                param_groups, lr=lr, momentum=train_config.get("momentum", 0.9)
            )
        else:  # Default to Adam
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        if (batch_idx + 1) % 100 == 0 and (args.local_rank == 0 or not args.distributed):
            ...
            logging.info(
                f"Epoch {epoch}, Batch {batch_idx+1}: loss={result['loss']:.4f}, elapsed={elapsed:.2f}s"
            )
            start_time = time.time()
...
def validate(
    model: nn.Module, data_loader: StreamingDataLoader, device: torch.device, args
) -> Dict[str, float]:
    """Validate the model.
    ...
    # Initialize metrics
    metrics = {"val_loss": 0.0, "val_accuracy": 0.0, "steps": 0}
...
def save_model(
    model: nn.Module, optimizer: torch.optim.Optimizer, epoch: int, metrics: Dict[str, float], args
):
