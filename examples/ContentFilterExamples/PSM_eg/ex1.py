...
def main():
    ...
    movies_file = Path("src/SANDBOX/dataset/ml-1m/movies.dat")
    ...
    lsa = PRO_LSA(n_components=min(20, len(movies_df['processed_genres'].tolist())))
    ...
    top_n = 5
    recommendations = lsa.recommend(query=movies_df.at[movie_index, 'processed_genres'], top_n=top_n)
    
    # Display recommendations
    logger.info(f"Top {top_n} recommendations for '{movie_title}':")
    for idx in recommendations:
        recommended_movie = movies_df.iloc[idx]
        print(f"{recommended_movie['movie_id']}:: {recommended_movie['title']}:: {recommended_movie['genres']}")
