    ...
    reels_data = pd.DataFrame(
        {
            ...
            "followed": (full_data["rating"] == 5).astype(int)
            & (np.random.random(len(full_data)) < 0.1),  # 10% follow
            ...
            if "timestamp" in full_data
            else np.random.randint(
                int(time.time()) - 365 * 86400, int(time.time()), len(full_data)  # Last year
            ),
...
class InstagramReelsRecommenderReal(BaseRecommender, Serializable):
    ...
    def recommend(
        self, user_id: int, top_k: int = 10, exclude_items: List[int] = None, **kwargs
    ) -> List[int]:
        ...
        candidate_reels = [rid for rid in self.reel_to_idx.keys() if rid not in seen_reels]
