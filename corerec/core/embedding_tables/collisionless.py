...
class CollisionlessEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        output_embeddings = torch.zeros(
            flat_ids.shape[0], self.embedding_dim, device=input_ids.device
        )

        # Compute embeddings from each hash function
        start_dim = 0
        for i, (table, seed) in enumerate(
                zip(self.embedding_tables, self.hash_seeds)):
            # Hash input IDs
            hashed_ids = torch.tensor([self._hash_id(
                id_val.item(), seed) for id_val in flat_ids], device=input_ids.device)
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        for chunk_start in range(0, flat_ids.shape[0], chunk_size):
            ...
            for i, (table, seed) in enumerate(
                ...
                # Add to output
                output_embeddings[
                    chunk_start:chunk_end, start_dim: start_dim + dim_size
                ] = embeddings
                start_dim += dim_size
