...
def run_streamlit_app():
    ...
    if platform_options:
        ...
        if selected_platform:
            platform_info = frontend_manager.get_available_platforms()[selected_platform]
            ...
            with col1:
                ...
                # Launch button
                if st.button(
                    f"🚀 Launch {platform_info['name']} Demo", key=f"launch_{selected_platform}"
                ):
                    ...
                    try:
                        ...
                        st.error(f"Error launching {platform_info['name']}: {str(e)}")
    ...
    with tab3:
        ...
        chart_data = pd.DataFrame(np.random.randn(20, 3), columns=["Spotify", "YouTube", "Netflix"])
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=3000, help="Port for web frontend (default: 3000)"
    )
    parser.add_argument(
        "--backend-only",
        action="store_true",
    ...
    parser.add_argument(
        "--no-browser", action="store_true", help="Don't automatically open browser (web mode only)"
    )

    # Handle Streamlit execution
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] in ["--mode", "streamlit"]):
