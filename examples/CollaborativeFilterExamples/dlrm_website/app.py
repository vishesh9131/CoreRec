...
def product_detail(merchant_id):
    ...
    related = random.sample(
        [m for m in merchants if m["id"] != merchant_id], min(4, len(merchants) - 1)
    )

    return render_template(
        "product.html", user=user, users=users, product=merchant, related=related
    )
...
def category(category):
    ...
    return render_template(
        "category.html", user=user, users=users, category=category, products=category_merchants
    )
    ...
    if recommender:
        try:
            ...
            if len(recommended) < count:
                ...
                remaining_merchants = [
                    m for m in merchants if m["id"] not in [r["id"] for r in recommended]
                ]
                if remaining_merchants:
                    random_recs = random.sample(
                        remaining_merchants, min(remaining, len(remaining_merchants))
                    )
