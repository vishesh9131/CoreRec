# ESCMM Tutorial: Entire Space Cross Multi-Task Model
...
**ESCMM** is a Neural Network Models model for recommendation systems. This model implements Entire Space Cross Multi-Task Model.

## How ESCMM Works

### Architecture

ESCMM uses a sophisticated architecture for recommendation tasks.
...
```python
from corerec.sandbox.collaborative_full.nn_base.ESCMM_base import ESCMM_base as ESCMM
...
```python
# Save model
model.save('artifacts/escmm_model')

# Load model
loaded = ESCMM_base.load('artifacts/escmm_model')
