...
class RLRMC(BaseRecommender):
    ...
    def _compute_loss(
        self, user_indices: torch.Tensor, item_indices: torch.Tensor, values: torch.Tensor
    ) -> float:
        """
        ...
        reg_loss = self.reg_param * (
            torch.norm(self.U, "fro") ** 2 + torch.norm(self.V, "fro") ** 2
        )
    ...
    def fit(self, interaction_matrix: csr_matrix, user_ids: List[int], item_ids: List[int]) -> None:
        ...
        for iteration in range(self.max_iter):
            ...
            # corerec/engines/collaborative/rlrmc.py (continued)
            # Update parameters using retraction
    ...
    def fit_from_interactions(
        self, user_ids: List[int], item_ids: List[int], ratings: Optional[List[float]] = None
    ) -> None:
        """
        ...
        interaction_matrix = csr_matrix(
            (ratings, (user_indices, item_indices)), shape=(self.n_users, self.n_items)
        )
