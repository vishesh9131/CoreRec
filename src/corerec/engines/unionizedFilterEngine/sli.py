...
class SLiRec(BaseRecommender):
    ...
    def _prepare_batch(
        self, sequences: List[List[int]], user_indices: List[int], batch_indices: List[int]
    ...
    def _forward(
        self, user_indices: torch.Tensor, item_sequences: torch.Tensor, seq_lengths: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def recommend(self, user_id: int, top_n: int = 10, exclude_seen: bool = True) -> List[int]:
        ...
        with torch.no_grad():
            ...
            if (
                not hasattr(self, "interaction_matrix")
                or len(self.interaction_matrix[user_idx].indices) == 0
            ):
                output = self.output_layer(
                    self.integration_layer(
                        torch.cat(
                            [torch.zeros(1, self.hidden_dim).to(self.device), user_emb], dim=1
                        )
                    )
                )
            else:
