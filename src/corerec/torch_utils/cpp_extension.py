...
def _join_rocm_home(*paths) -> str:
    ...
    if ROCM_HOME is None:
        raise OSError(
            "ROCM_HOME environment variable is not set. " "Please set it to your ROCm install root."
        )
    elif IS_WINDOWS:
...
def get_compiler_abi_compatibility_and_version(compiler) -> Tuple[bool, TorchVersion]:
    ...
    try:
        if IS_LINUX:
            ...
            match = re.search(
                r"(\d+)\.(\d+)\.(\d+)", compiler_info.decode(*SUBPROCESS_DECODE_ARGS).strip()
            )
...
def _check_cuda_version(compiler_name: str, compiler_version: TorchVersion) -> None:
    ...
    cuda_version_str = (
        subprocess.check_output([nvcc, "--version"]).strip().decode(*SUBPROCESS_DECODE_ARGS)
    )
...
def _get_cuda_arch_flags(cflags: Optional[List[str]] = None) -> List[str]:
    ...
    if not _arch_list:
        ...
        for i in range(torch.cuda.device_count()):
            ...
            supported_sm = [
                int(arch.split("_")[1]) for arch in torch.cuda.get_arch_list() if "sm_" in arch
            ]
...
def _get_num_workers(verbose: bool) -> Optional[int]:
    ...
    if max_jobs is not None and max_jobs.isdigit():
        if verbose:
            print(
                f"Using envvar MAX_JOBS ({max_jobs}) as the number of workers...", file=sys.stderr
            )
        return int(max_jobs)
    if verbose:
        print(
