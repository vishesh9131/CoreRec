...
class GeoMLC(BaseRecommender):
    ...
    def _init_model(self) -> None:
        ...
        self.optimizer = optim.Adam(
            self.model.parameters(), lr=self.learning_rate, weight_decay=self.regularization
        )

    def _prepare_data(
        self, user_ids: List[int], item_ids: List[int], ratings: List[float]
    ...
    def recommend(self, user_id: int, top_n: int = 10, exclude_seen: bool = True) -> List[int]:
        ...
        validate_user_id(user_id, self.user_mapping if hasattr(self, "user_mapping") else {})
        ...
        self.model.eval()
        with torch.no_grad():
            predictions = self.model(user_tensor.expand(self.n_items), all_items).cpu().numpy()

        # Get seen items if needed
        if exclude_seen:
            # Track seen items during training
            seen_items = set()
            if hasattr(self.model, 'interaction_matrix'):
                user_idx = self.user_mapping[user_id]
                # Get all items this user has interacted with
                seen_item_indices = self.model.interaction_matrix[user_idx].nonzero().cpu().numpy().flatten()
                seen_items = set(seen_item_indices.tolist())

            # Set predictions for seen items to -inf
            for item_idx in seen_items:
                if 0 <= item_idx < len(predictions):
...
class GeoMLCModel(nn.Module):
    ...
    def project_to_manifold(self) -> None:
        ...
        with torch.no_grad():
            if self.manifold_type == "hyperbolic":
                ...
                self.user_embeddings.weight.data = F.normalize(
                    self.user_embeddings.weight.data, p=2, dim=1
                )
                self.item_embeddings.weight.data = F.normalize(
    ...
    def _manifold_distance(self, user_emb: torch.Tensor, item_emb: torch.Tensor) -> torch.Tensor:
        ...
        if self.manifold_type == "euclidean":
            ...
            denominator = (1 - user_norm_sq) * (
                1 - item_norm_sq
            ) + 1e-6  # Add epsilon to avoid division by zero
