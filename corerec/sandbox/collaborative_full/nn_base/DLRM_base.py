...
class FeatureEmbedding(nn.Module):
    ...
    def __init__(self, field_dims: List[int], embed_dim: int):
        ...
        self.embedding = nn.ModuleList(
            [nn.Embedding(num_embeddings=dim, embedding_dim=embed_dim) for dim in field_dims]
        )
    ...
    def forward(self, x_sparse: torch.Tensor) -> torch.Tensor:
        ...
        """
        sparse_embeds = [self.embedding[i](x_sparse[:, i]) for i in range(x_sparse.size(1))]
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        if output_layer:
            layers.append(
                nn.Linear(layer_dims[-2] if len(layer_dims) > 1 else input_dim, layer_dims[-1])
            )
...
class DLRMModel(nn.Module):
    ...
    def __init__(
        ...
        # Top MLP
        self.top_mlp = MLP(
            input_dim=top_input_dim, layer_dims=top_mlp_dims, dropout=dropout, batchnorm=batchnorm
        )
    ...
    def forward(self, x_sparse: torch.Tensor, x_dense: torch.Tensor) -> torch.Tensor:
        ...
        sparse_embeds = self.sparse_embedding(
            x_sparse
        )  # (batch_size, num_sparse_fields, embed_dim)
        ...
        combined = torch.cat(
            [sparse_embeds, dense_out.unsqueeze(1)], dim=1
...
class DLRM_base(BaseRecommender):
    """
    ...
    def _build_model(self):
        ...
        if self.verbose:
            self.logger.info(
                f"Built DLRM model with {len(self.field_dims)} categorical fields and {self.dense_dim} dense features"
            )
    ...
    def _prepare_batch(
        ...
        return (
            sparse_features.to(self.device),
            dense_features.to(self.device),
