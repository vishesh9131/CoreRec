...
class PointWiseFeedForward(nn.Module):
    ...
    def forward(self, inputs):
        outputs = self.dropout2(
            self.conv2(self.activation(self.dropout1(self.conv1(inputs.transpose(-1, -2)))))
        )
...
class SASRecModel(nn.Module):
    ...
    def __init__(
        ...
        if item_embedding_init is not None:
            assert item_embedding_init.shape == (
                n_items + 1,
                hidden_units,
        ...
        for _ in range(num_blocks):
            ...
            self.feed_forwards.append(PointWiseFeedForward(hidden_units, dropout_rate, activation))
    ...
    def _get_sinusoidal_encoding(self, max_seq_len, hidden_units):
        ...
        def get_posi_angle_vec(position):
            return [
                get_position_angle(position, hid_j, hidden_units) for hid_j in range(hidden_units)
            ]
...
class SASRec(BaseCorerec):
    ...
    def fit(
        self, interaction_matrix, user_ids: List[int], item_ids: List[int], validation_data=None
    ):
        """
        ...
        """
        self.logger.info(
            f"Training {self.name} model with {len(user_ids)} users and {len(item_ids)} items"
        )
        ...
        if self.item_popularity_bias:
            self.item_popularity = (
                np.asarray(interaction_matrix.sum(axis=0)).flatten() + 1
            )  # +1 smoothing
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(0, n_train, self.batch_size):
                ...
                batch_negatives = [
                    torch.LongTensor(negs).to(self.device) for negs in batch_negatives
                ]
                ...
                if self.loss_type == "bpr":
                    ...
                    neg_scores_list = [
                        self.model.predict(last_emb, negs) for negs in batch_negatives
                    ]
                    ...
                    for neg_scores in neg_scores_list:
                        ...
                        if self.item_popularity_bias:
                            ...
                            neg_correction = (
                                torch.from_numpy(self.item_popularity[negs.cpu().numpy() - 1])
                                .float()
                                .to(self.device)
                            )
            ...
            if self.save_checkpoints:
                checkpoint_path = os.path.join(
                    self.checkpoint_dir, f"{self.name}_epoch_{epoch+1}.pt"
                )
    ...
    def export_item_embeddings(self, filepath=None):
        ...
        export_data = {"embeddings": embeddings, "index_to_item": self.index_to_item}
    ...
    def get_attention_weights(self, user_id):
        ...
        with torch.no_grad():
            _, attention_weights = self.model(
                input_seq, padding_mask, return_attention_weights=True
            )
