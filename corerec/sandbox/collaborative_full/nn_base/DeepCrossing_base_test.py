...
from corerec.engines.collaborative.nn_base.DeepCrossing_base import (
    DeepCrossing_base,
    ResidualUnit,
)
import pickle
...
class PatchedDeepCrossing_base(DeepCrossing_base):
    ...
    def fit(self, interactions):
        ...
        for epoch in range(self.num_epochs):
            ...
            outputs = self.model(
                torch.randn(len(interactions), model_input_dim, device=self.device)
            )
        ...
        try:
            ...
            try:
                recommendations_with_features = self.model.recommend(
                    user, top_n=5, features=features
                )
    ...
    def test_update_incremental(self):
        ...
        new_interactions.append(
            (
                self.users[0],
                new_items[0],
                {"category": "books", "price": 25, "rating": 4, "is_new": False, "discount": 0.2},
            )
        )
        ...
        new_interactions.append(
            (
                new_users[0],
                self.items[0],
                {"category": "clothing", "price": 50, "rating": 3, "is_new": True, "discount": 0.3},
            )
        )
        # New user, new item
        new_interactions.append(
