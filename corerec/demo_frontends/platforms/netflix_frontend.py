...
class NetflixFrontend(BaseFrontend):
    ...
    def render_sidebar(self):
        ...
        sort_by = st.selectbox(
            "Sort by", ["Recommended", "Netflix Score", "IMDB Rating", "Release Year", "Title"]
        )
    ...
    def _get_content_based_recommendations(
        self, df: pd.DataFrame, liked_content: set
    ) -> pd.DataFrame:
        ...
        if not liked_df.empty:
            ...
            df["genre_boost"] = (
                df["genre"].map(lambda x: preferred_genres.get(x, 0) / len(liked_content)).fillna(0)
            )

            df["type_boost"] = (
                df["type"].map(lambda x: preferred_types.get(x, 0) / len(liked_content)).fillna(0)
            )
