...
if HAS_TABULATE:
    ...
    def benchmark_compile(
        ...
        if backend:
            try:
                torch._dynamo.reset()
                compile_counter_with_backend = CompileCounterWithBackend(
                    backend)
                ...
                if compile_counter_with_backend.frame_count == 0:
                    raise RuntimeError(
                        "No compilation occurred during benchmarking.")

                if compile_counter_with_backend.frame_count > 1:
                    raise RuntimeError(
                        "Recompilation occurred during benchmarking.")
        ...
        compilation_time = round(
            compilation_time,
            2) if compilation_time else None
    ...
    def bench_all(
        ...
        table.append([("Training" if optimizer else "Inference"),
                      "Eager", "-", "-", f"{eager_time} ms"])

        for backend in torch._dynamo.list_backends():
            if backend == "inductor":
                mode_options = cast(List[Optional[str]], list(
                    torch._inductor.list_mode_options().keys())) + [None]
