...
class PositionAwareMFRecommender(MatrixFactorizationBase):
    def __init__(self, num_factors=20, learning_rate=0.01, reg_user=0.02, reg_item=0.02,
                 epochs=20, early_stopping_rounds=5, n_threads=4):
        super().__init__(num_factors, learning_rate, reg_user, reg_item, epochs, early_stopping_rounds, n_threads)

    def fit(self, interaction_matrix: csr_matrix, user_ids: List[int], item_ids: List[int], positions: Optional[List[int]] = None):
        """
