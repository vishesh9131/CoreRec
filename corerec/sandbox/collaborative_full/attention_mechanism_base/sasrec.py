...
class SASRec(BaseRecommender):
    ...
    def _create_mappings(
        self, user_ids: List[int], item_ids: List[int], timestamps: List[int]
    ) -> None:
        """
    ...
    def _prepare_batch(
        self, batch_data: List[Tuple[int, List[int], int]]
    ...
    def fit(self, user_ids: List[int], item_ids: List[int], timestamps: List[int]) -> None:
        ...
        for epoch in range(self.num_epochs):
            ...
            if (epoch + 1) % 10 == 0:
                if self.verbose:
                    logger.info(
                        f"Epoch {epoch + 1}/{self.num_epochs}, Loss: {epoch_loss / n_train:.4f}"
                    )
