...
class SUMModel(BaseRecommender):
    ...
    def _build_model(self) -> None:
        ...
        self.transform_matrices = nn.Parameter(
            torch.randn(
                self.num_interests, self.embedding_dim, self.interest_dim, device=self.torch_device
        ...
        self.output_projection = nn.Linear(self.interest_dim, self.embedding_dim).to(
            self.torch_device
        )
        ...
        self.optimizer = torch.optim.Adam(
            model_params, lr=self.learning_rate, weight_decay=self.l2_reg
        )
    ...
    def _dynamic_routing(self, item_embeddings: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            u_hat = torch.matmul(
                item_embeddings_expanded, self.transform_matrices
        ...
        b = torch.zeros(
            batch_size, self.num_interests, seq_len, device=self.torch_device
        ).unsqueeze(
            -1
    ...
    def _sequence_attention(
        self, item_embeddings: torch.Tensor, mask: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        weighted_sum = torch.sum(
            attention_weights * item_embeddings, dim=1
        )  # (batch_size, embedding_dim)

        return weighted_sum

    def _interest_level_attention(
        self, interests: torch.Tensor, target_item_emb: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        target_projection = torch.bmm(
            ...
            self.transform_matrices.mean(dim=0)
            .unsqueeze(0)
            .expand(target_item_emb.size(0), -1, -1),  # (batch_size, embedding_dim, interest_dim)
        ).squeeze(
            1
        )  # (batch_size, interest_dim)
        ...
        similarities = torch.bmm(
            ...
            -1
        )  # (batch_size, num_interests)

        # Apply softmax
        attention_weights = F.softmax(similarities, dim=1).unsqueeze(
            -1
        )  # (batch_size, num_interests, 1)
    ...
    def _forward(
        self, item_seq: torch.Tensor, seq_mask: torch.Tensor, target_item: torch.Tensor = None
    ) -> torch.Tensor:
        """
        ...
        interests = self._dynamic_routing(
            seq_embeddings, seq_mask
        )  # (batch_size, num_interests, interest_dim)
        ...
        if target_item is not None:
            ...
            interest_vector = self._interest_level_attention(
                interests, target_emb
            )  # (batch_size, interest_dim)
        else:
            # During inference, use attention over interests
            interest_scores = self.interest_attention(interests.view(-1, self.interest_dim)).view(
                -1, self.num_interests
            )
            interest_weights = F.softmax(interest_scores, dim=1).unsqueeze(
                -1
            )  # (batch_size, num_interests, 1)
            interest_vector = torch.sum(
                interest_weights * interests, dim=1
            )  # (batch_size, interest_dim)
        ...
        try:
            for epoch in range(self.epochs):
                ...
                for i in range(n_batches):
                    ...
                    batch_seq = torch.zeros((len(batch_data), max_seq_len), dtype=torch.long).to(
                        self.torch_device
                    )
                    batch_mask = torch.zeros((len(batch_data), max_seq_len), dtype=torch.float).to(
                        self.torch_device
                    )
                    batch_target = torch.tensor(
                        [target for _, target in batch_data], dtype=torch.long
    ...
    def load_model(cls, filepath: str, device: str = "auto") -> "SUMModel":
        ...
        instance.transform_matrices.data = model_dict["transform_matrices"].to(
            instance.torch_device
        )
