# # corerec/engines/collaborative/als_recommender.py
"""
    ...
    def fit(self, interaction_matrix: csr_matrix, user_ids: List[int], item_ids: List[int]) -> None:
        ...
        for iteration in range(self.iterations):
            ...
            for u in range(num_users):
                ...
                A = (
                    factors_i.T.dot(np.diag(conf_u)).dot(factors_i)
            ...
            for i in range(num_items):
                ...
                # Build the right side of the equation
                A = (
                    factors_u.T.dot(np.diag(conf_i)).dot(factors_u)
                    + np.eye(self.num_factors) * self.regularization
                )
            ...
            if self.verbose:
                logger.info(f"Iteration {iteration+1}/{self.iterations} completed")
