# corerec/engines/collaborative/rbm.py
import numpy as np
import torch
...
class RBM(BaseRecommender):
    ...
    def _contrastive_divergence(
        ...
        # Compute gradients
        dW = (
            torch.matmul(v_data.t(), h_prob_data) - torch.matmul(v_prob_model.t(), h_prob_model)
        ).t()
    ...
    def _update_parameters(
        self, dW: torch.Tensor, dv_bias: torch.Tensor, dh_bias: torch.Tensor
    ) -> None:
        """
