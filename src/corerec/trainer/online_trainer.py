...
class OnlineTrainer:
    ...
    def __init__(
        ...
        self.gradient_accumulator = GradientAccumulator(
            model=model, accumulation_steps=config.get("accumulation_steps", 1)
        )
    ...
    def _training_loop(self):
        ...
        while not self.stop_training.is_set():
            ...
            while (
                self.data_queue.qsize() < min_batch_size
                and time.time() - start_wait_time < max_wait_time
                and not self.stop_training.is_set()
            ):
                time.sleep(0.1)
            ...
            if self.metrics["updates"] % 10 == 0:
                ...
                self.logger.info(
                    f"Update {self.metrics['updates']}: avg_loss={avg_loss:.4f}, queue_size={self.data_queue.qsize()}"
                )
