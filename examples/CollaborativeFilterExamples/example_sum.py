    ...
    for user_id in user_ids:
        ...
        for i in range(n_user_interactions):
            ...
            if random.random() < 0.8:
                ...
                category_items = [
                    item for item, cats in item_categories.items() if category in cats
                ]
...
def analyze_user_interests(model, user_id, top_n=5):
    ...
    with torch.no_grad():
        ...
        # Get interest scores
        interest_scores = model.interest_attention(interests.view(-1, model.interest_dim)).view(
            -1, model.num_interests
        )
