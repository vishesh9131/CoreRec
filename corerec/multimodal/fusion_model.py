...
class AttentionFusion(ModalityFusion):
    ...
    def __init__(self, modality_dims: Dict[str, int], output_dim: int):
        ...
        self.projections = nn.ModuleDict({modality: nn.Linear(
            dim, output_dim) for modality, dim in modality_dims.items()})
        ...
        self.attention = nn.Sequential(
            nn.Linear(
                output_dim, 64), nn.Tanh(), nn.Linear(
                64, 1))
    ...
    def forward(self,
        ...
        for modality, embedding in modality_embeddings.items():
            projected_embeddings[modality] = self.projections[modality](
                embedding)
        ...
        attention_scores = self.attention(
            stacked).squeeze(-1)  # [batch_size, num_modalities]
...
class GatingFusion(ModalityFusion):
    ...
    def forward(self,
        ...
        gated_embeddings = {}
        for modality in self.modality_dims:
            gated_embeddings[modality] = projected_embeddings[modality] * \
                gates[modality]
...
class MultimodalFusionModel(BaseModel):
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        prediction_hidden_dims = config.get(
    ...
    def forward(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        if not has_text and not has_vision:
            raise ValueError(
                "Batch must contain at least one modality (text or images)")
        ...
        if has_text:
            if "text_features" in batch:
                ...
                # Encode text
                modality_embeddings["text"] = self.text_encoder.encode(
                    batch["text"])
        ...
        if has_vision:
            if "image_features" in batch:
                ...
                # Encode images
                modality_embeddings["vision"] = self.vision_encoder.encode(
                    batch["images"])
        ...
        if "text" not in modality_embeddings:
            ...
            modality_embeddings["text"] = torch.zeros(
                batch_size, text_dim, device=self.device)
        ...
        if "vision" not in modality_embeddings:
            ...
            modality_embeddings["vision"] = torch.zeros(
                batch_size, vision_dim, device=self.device)
