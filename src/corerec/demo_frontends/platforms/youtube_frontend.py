...
class YouTubeFrontend(BaseFrontend):
    ...
    def render_sidebar(self):
        ...
        duration_filter = st.selectbox(
            "Video Duration", ["Any", "Short (< 4 min)", "Medium (4-20 min)", "Long (> 20 min)"]
        )
        self.set_session_value("duration_filter", duration_filter)

        # Quality filter
        quality_filter = st.selectbox(
            "Video Quality", ["Any", "High Quality (>0.7)", "Medium Quality (>0.4)", "Any Quality"]
        )
        self.set_session_value("quality_filter", quality_filter)

        # Sort by
        sort_by = st.selectbox(
            "Sort by", ["Recommended", "Most Views", "Most Recent", "Most Liked", "Trending"]
        )
    ...
    def get_recommendations(self, user_id: int, num_recommendations: int = 12) -> List[Dict]:
        ...
        if sort_by == "Most Views":
            ...
            df["trending_score"] = (
                df["views"] * 0.4 + df["likes"] * 10 * 0.3 + df["engagement_rate"] * 100000 * 0.3
            )
            ...
            if liked_videos:
                ...
                df["trending_score"] = df["views"] * 0.5 + df["likes"] * 10 * 0.5
                df = df.sort_values("trending_score", ascending=False)
    ...
    def _get_content_based_recommendations(
        self, df: pd.DataFrame, liked_videos: set
    ) -> pd.DataFrame:
        ...
        if not liked_df.empty:
            ...
            df["category_boost"] = (
                df["category"]
                .map(lambda x: preferred_categories.get(x, 0) / len(liked_videos))
                .fillna(0)
            )
