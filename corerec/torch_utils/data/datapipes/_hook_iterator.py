...
def _check_iterator_valid(
        datapipe,
        iterator_id,
        next_method_exists=False) -> None:
    r"""
    ...
    if next_method_exists:
        ...
        if datapipe._valid_iterator_id is not None and datapipe._valid_iterator_id != 0:
            ...
            raise RuntimeError(
                _gen_invalid_iterdatapipe_msg(datapipe) +
                extra_msg +
                _feedback_msg)
...
def _set_datapipe_valid_iterator_id(datapipe):
    ...
    if hasattr(
            datapipe,
            "_is_child_datapipe") and datapipe._is_child_datapipe is True:
        if hasattr(datapipe, "_set_main_datapipe_valid_iterator_id"):
            # reset() is called within this method when appropriate
...
def hook_iterator(namespace):
    ...
    if inspect.isgeneratorfunction(func):
        ...
        # Regardless if `__next__` exists or not, `__iter__` needs a wrapper to
        # track the number of valid iterators
        @functools.wraps(func)
