...
class GraphTransformerV2(nn.Module):
    ...
    def forward(self, x, adjacency_matrix, graph_metrics, weights=None):
        ...
        if adjacency_matrix.size(
                0) != batch_size or adjacency_matrix.size(1) != batch_size:
        ...
        try:
            ...
            # Neighborhood Similarity (modified to handle potential dimension
            # issues)
            try:
            ...
            if graph_metrics.dim() == 2:
                ...
                graph_metrics_projected = self.project_graph_metrics(
                    graph_metrics, input_dim)
            ...
            if self.use_weights and weights is not None:
                ...
                for i, weight in enumerate(weights.T):
                    weighted_x += self.weight_linears[i](x) * \
                        weight.unsqueeze(1)
                x = weighted_x
            else:
            ...
            x = self.transformer_encoder(x.unsqueeze(1)).squeeze(
                1)  # Adjust for transformer input
    ...
    def project_graph_metrics(self, graph_metrics, target_dim):
        ...
        if graph_metrics.size(1) < target_dim:
            ...
            repeats = (target_dim + graph_metrics.size(1) -
                       1) // graph_metrics.size(1)
...
class TestingGraphTransformersAugAttention(Module):
    ...
    def forward(self, x, adjacency_matrix, graph_metrics, weights=None):
        # Process adjacency matrix and compute direct, neighborhood, and graph
        # structure scores
        adjacency_matrix = adjacency_matrix.float()
