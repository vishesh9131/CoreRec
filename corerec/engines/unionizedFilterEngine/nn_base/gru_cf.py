...
class GRUNet(nn.Module):
    ...
    def forward(
        self, item_seq: torch.Tensor, hidden: Optional[torch.Tensor] = None
...
class GRUCF(BaseRecommender):
    ...
    def __init__(
        ...
        def _prepare_sequences(
            self, user_ids: List[int], item_ids: List[int], timestamps: Optional[List[int]] = None
        ) -> None:
            """
        ...
        def _generate_training_batches(self) -> List[Tuple[torch.Tensor, torch.Tensor]]:
            ...
            for i in range(0, len(sequences), self.batch_size):
                ...
                batch_input = torch.tensor(
                    [seq[:-1] for seq in padded_sequences], dtype=torch.long, device=self.device
                )
        ...
        def recommend(self, user_id: int, top_n: int = 10, exclude_seen: bool = True) -> List[int]:
            ...
            with torch.no_grad():
                ...
                _, top_indices = torch.topk(
                    predictions[0], k=min(top_n + 10, predictions.size(1) - 1)
                )
