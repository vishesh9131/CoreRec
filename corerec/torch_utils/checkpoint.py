...
def check_backward_validity(inputs: Iterable[Any]) -> None:
    if not any(
        inp.requires_grad for inp in inputs if isinstance(
            inp,
            torch.Tensor)):
        warnings.warn(
...
#
# To consider:  maybe get_device_states and set_device_states should
# reside in torch/random.py?
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        ...
        # Accommodates the (remote) possibility that autocast is enabled for
        # cpu AND gpu.
        ...
        if preserve_rng_state:
            ...
            # we have no way to anticipate this will happen before we run the
            # function.)
            ctx.had_device_in_fwd = False
            device_module = _get_device_module(ctx.device_type)
            if getattr(device_module, "_initialized", False):
                ctx.had_device_in_fwd = True
                ctx.fwd_devices, ctx.fwd_device_states = get_device_states(
                    *args)
    ...
    def backward(ctx, *args):
        ...
        grads = tuple(inp.grad if isinstance(inp, torch.Tensor)
                      else None for inp in detached_inputs)
    ...
    def check_recomputed_tensors_match(self, gid):
        ...
        for idx, weak_holder in enumerate(self.weak_holders):
            ...
            if x_meta != self.metadata_fn(recomputed_x):
                nb_meta_different.append(
                    (idx, x_meta, self.metadata_fn(recomputed_x)))
...
def _get_debug_context_and_cb(
    ...
    def unpack_error_cb(e: CheckpointError):
        def get_str_tb(label, capture_logs):
            ...
            for i, (log, tb) in enumerate(
                    zip(capture_logs.logs, capture_logs.tbs)):
                out += f"{log}   ({i + 1} of {total_len} in {label})\n\n"
                found_torch_dispatch = False
                for line in tb:
                    # Start printing stack trace only after __torch_dispatch__
                    # is found
        ...
        raise CheckpointError(
            _checkpoint_error_template.format(
                forward_traces=get_str_tb(
                    "original", capture_logs_fwd), recompute_traces=get_str_tb(
                    "recompute", capture_logs_recompute), forward_ops="\n".join(
                    capture_logs_fwd.logs), recompute_ops="\n".join(
                        capture_logs_recompute.logs), )) from e

    def context_fn():
        return capture_logs_fwd.get_context_manager(
...
class _checkpoint_hook(torch.autograd.graph.saved_tensors_hooks):
    def __init__(self, frame):
        ...
        def unpack_hook(holder):
            ...
            if gid == -1:
                # generate a temporary id if we trigger unpack outside of a
                # backward call
                gid = int(uuid.uuid4())
...
class _VersionWrapper:
    ...
    def __init__(self, val):
        ...
        self.version: Optional[int] = val._version if isinstance(
            val, torch.Tensor) else None
    ...
    def get_val(self, allow_cache_entry_mutation):
        if self.version is not None and not allow_cache_entry_mutation:
            if self.val._version != self.version:
                # Can we give user a stack trace of where the mutation
                # happened?
                raise RuntimeError(
...
class _CachingTorchDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if is_compiling:
            # Overwrite each node's "recompute" tag to add in the user
            # annotation.
        ...
        if policy in (CheckpointPolicy.MUST_SAVE,
                      CheckpointPolicy.PREFER_SAVE) or is_compiling:
            self.storage[func].append(
                tree_map(
                    lambda x: _VersionWrapper(
                        _maybe_detach(
                            x,
                            any_ret_has_alias_info)),
                    out))
        return out
...
class _CachedTorchDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        policy = self.policy_fn(
            SelectiveCheckpointContext(
                is_recompute=True), func, *args, **kwargs)
        if isinstance(policy, bool):
            policy = _policy_from_bool(policy)
...
def create_selective_checkpoint_contexts(
    ...
    if isinstance(policy_fn_or_list, list):
        ...
        raise TypeError(
            "policy_fn_or_list must be either a function or a list of ops.")

    storage: Dict[Any, List[Any]] = defaultdict(list)
    return (
        _CachingTorchDispatchMode(
            policy_fn, storage), _CachedTorchDispatchMode(
            policy_fn, storage, allow_cache_entry_mutation), )
...
def _checkpoint_without_reentrant_generator(
    ...
    **kwargs,
    ...
    # Accommodates the (remote) possibility that autocast is enabled for cpu
    # AND gpu.
    ...
    def recompute_fn(*inputs):
        ...
        with torch.random.fork_rng(
            ...
            device_autocast_ctx = (
                torch.amp.autocast(device_type=device_type, **device_autocast_kwargs)
                if torch.amp.is_autocast_available(device_type)
                else contextlib.nullcontext()
            )
            # type: ignore[attr-defined]
