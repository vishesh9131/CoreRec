...
__all__ = ["rename_privateuse1_backend",
...
def _normalization_device(
    ...
    if isinstance(device, torch.device):
        if device.type != custom_backend_name:
            raise RuntimeError(
                f"Invalid device, must be {custom_backend_name} device")
...
def _generate_tensor_methods_for_privateuse1_backend(
        custom_backend_name: str) -> None:
    @property  # type: ignore[misc]
    def wrap_tensor_backend(self: torch.Tensor) -> bool:
        if has_torch_function_unary(self):
            # TODO mypy doesn't support @property, see:
            # https://github.com/python/mypy/issues/6185
            return handle_torch_function(
                wrap_tensor_backend.__get__, (self,), self)  # type: ignore[attr-defined]
        return self.device.type == custom_backend_name

    _check_register_once(torch.Tensor, f"is_{custom_backend_name}")
    # type: ignore[attr-defined]
    wrap_tensor_backend.fget.__name__ = f"is_{custom_backend_name}"
    ...
    def wrap_tensor_to(
        ...
        **kwargs,
        ...
        if has_torch_function_unary(self):
            return handle_torch_function(
                wrap_tensor_to, (self,), self, device=device, non_blocking=False, **kwargs)
...
def _generate_module_methods_for_privateuse1_backend(
        custom_backend_name: str) -> None:
    ...
    def wrap_module_to(
        self: torch.nn.modules.module.T, device: Optional[Union[int, torch.device]] = None
...
def _generate_storage_methods_for_privateuse1_backend(
    ...
    def wrap_storage_to(self, device=None, non_blocking=False):
        ...
        # but it depends on the extended function, so this part is temporarily
        # omitted in the automatic generation.
        device_idx = _normalization_device(custom_backend_name, device)
        ...
        if getattr(self, f"is_{custom_backend_name}"):
            ...
            if self.get_device() == device_idx:
                return self
        # For sparse storage, custom need to extend the implementation by
        # themselves.
        if self.is_sparse:
            raise RuntimeError(
        ...
        untyped_storage = torch.UntypedStorage(
            self.size(), device=torch.device(
                f"{custom_backend_name}:{device_idx}"))
    ...
    def wrap_typed_storage_to(
            self: torch.storage.TypedStorage,
            device=None,
            non_blocking=False,
...
def generate_methods_for_privateuse1_backend(
    ...
    if for_packed_sequence:
        _generate_packed_sequence_methods_for_privateuse1_backend(
            custom_backend_name)
...
def _get_custom_mod_func(func_name: str):
    ...
    """
    assert isinstance(
        func_name, str), f"func_name must be `str`, but got `{
        type(func_name)}`."
    ...
    if custom_device_mod is None or function is None:
        ...
        message += (
            f"module with `torch._register_device_module('{backend_name}', BackendModule)`. And "
        )
        message += (
            f"BackendModule needs to have the following API's:\n `{func_name}(*args, **kwargs)`. \n"
        )
