...
class RemovableHandle:
    ...
    def __getstate__(self):
        if self.extra_dict_ref is None:
            ...
            return (self.hooks_dict_ref(), self.id, tuple(ref()
                    for ref in self.extra_dict_ref))
...
class BackwardHook:
    ...
    def _apply_on_tensors(self, fn, args):
        ...
        new_tensors = torch.nn.modules._functions.BackwardHookFunction.apply(
            *tensors)
    ...
    def setup_output_hook(self, args):
        def fn(grad_fn):
            def hook(_, grad_output):
                ...
                # We need to be able to clear self.grad_outputs but also return
                # it
                ...
                if self.input_tensors_index is None:
                    ...
                    for user_hook in self.user_hooks:
                        res = user_hook(
                            self.module, grad_inputs, self.grad_outputs)
                        if res is not None and not (
                            isinstance(
                                res, tuple) and all(
                                el is None for el in res)):
                            raise RuntimeError(
                ...
                if local_grad_outputs is not None:
                    ...
                    return tuple(local_grad_outputs[i]
                                 for i in self.output_tensors_index)
