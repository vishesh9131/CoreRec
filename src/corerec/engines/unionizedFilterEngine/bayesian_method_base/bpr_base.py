...
class BPRBase(BaseRecommender):
    ...
    def _compute_loss(self, user_idx: int, pos_idx: int, neg_idx: int) -> float:
        ...
        # Compute scores
        pos_score = (
            np.dot(self.user_factors[user_idx], self.item_factors[pos_idx])
            + self.item_bias[pos_idx]
        )
        neg_score = (
            np.dot(self.user_factors[user_idx], self.item_factors[neg_idx])
    ...
    def _update_factors(self, user_idx: int, pos_idx: int, neg_idx: int) -> None:
        ...
        # Compute gradients
        grad_user = (
            sigmoid * (self.item_factors[neg_idx] - self.item_factors[pos_idx])
            + self.regularization * self.user_factors[user_idx]
        )

        grad_pos_item = (
            sigmoid * (-self.user_factors[user_idx])
            + self.regularization * self.item_factors[pos_idx]
        )

        grad_neg_item = (
            sigmoid * self.user_factors[user_idx] + self.regularization * self.item_factors[neg_idx]
        )
