# FLEN Tutorial: Feature-aware Local Encoding Network
...
**FLEN** is a Neural Network Models model for recommendation systems. This model implements Feature-aware Local Encoding Network.

## How FLEN Works

### Architecture

FLEN uses a sophisticated architecture for recommendation tasks.
...
```python
from corerec.sandbox.collaborative_full.nn_base.FLEN_base import FLEN_base as FLEN
...
```python
# Save model
model.save('artifacts/flen_model')

# Load model
loaded = FLEN_base.load('artifacts/flen_model')
