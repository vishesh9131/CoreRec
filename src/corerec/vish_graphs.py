...
def generate_connections(num_people, i, p_strong, p_weak):
    ...
    weak_connections = (
        np.where((connections >= p_strong) & (connections < p_strong + p_weak))[0] + i + 1
    )
...
def find_top_nodes(matrix, num_nodes=3):
    ...
    top_nodes = sorted(range(len(relation_counts)), key=lambda i: relation_counts[i], reverse=True)[
        :num_nodes
    ]
    print(
...
def draw_graph(
    ...
    for i, j in G.edges():
        ...
        nx.draw_networkx_edges(
            G, pos, edgelist=[(i, j)], width=1.0, alpha=edge_alpha, edge_color=edge_color
        )
