...
def hierarchical_pickle(data):
    ...
    if isinstance(data, torch.utils.show_pickle.FakeObject):
        ...
        if typename.startswith(
            ("__torch__.",
             "torch.jit.LoweredWrapper.",
             "torch.jit.LoweredModule.")):
            assert data.args == ()
            return {
        ...
        if typename == "builtin.UnicodeDecodeError":
            ...
            # TODO: Undo at least that second hack.  We should support string
            # states.
            return {
                "__module_type__": typename,
    ...
    with zipfile.ZipFile(path_or_file) as zf:
        ...
        def get_pickle(name):
            ...
            with zf.open(path_prefix + f"/{name}.pkl") as handle:
                raw = torch.utils.show_pickle.DumpUnpickler(
                    handle, catch_invalid_utf8=True).load()
        ...
        for zi in zf.infolist():
            ...
            if (
                ...
                def parse_new_format(line):
                    # (0, (('', '', 0), 0, 0))
                    num, ((text_indexes, fname_idx, offset),
                          start, end), tag = line
                    text = "".join(
                        text_table[x] for x in text_indexes)  # type: ignore[index]
            ...
            for di, di_next in zip(debug_info, debug_info[1:]):
                ...
                code_parts.append(
                    [text.decode("utf-8"), ist(s_file), s_line, ist(s_text), s_start, s_end]
                )
        ...
        for zi in zf.infolist():
            ...
            with zf.open(zi) as handle:
                ...
                # we could get away with just unpickling, but this should be
                # safer.
            ...
            # because pickles with shared structure can explode in size during
            # rendering.
            if (
...
def get_inline_skeleton():
    ...
    for js_module in ["preact", "htm"]:
        js_lib = importlib.resources.read_binary(
            __package__, f"{js_module}.mjs")
        js_url = "data:application/javascript," + urllib.parse.quote(js_lib)
        js_code = js_code.replace(
            f"https://unpkg.com/{js_module}?module", js_url)
