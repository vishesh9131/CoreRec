...
def test_hanconv_with_semantic_attention(sample_hetero_data):
    ...
    out_dict, semantic_attn_dict = conv(
    ...
    assert (
        semantic_attn_dict["paper"] is not None
    ), "Semantic attention weights for 'paper' should not be None"
...
def test_hanconv_multiple_heads(sample_hetero_data):
    ...
    conv = HANConv(
        in_channels={"paper": 2, "author": 2}, out_channels=4, metadata=metadata, heads=2
    )
    ...
    # Check output dimensions
    assert out_dict["paper"].size() == (
        4,
        4,
    ), "Output dimensions for 'paper' with multiple heads are incorrect"
...
def test_hanconv_dropout(sample_hetero_data):
    ...
    conv = HANConv(
        in_channels={"paper": 2, "author": 2}, out_channels=4, metadata=metadata, dropout=0.5
    )
    ...
    assert out_dict["paper"].size() == (
        4,
        4,
    ), "Output dimensions for 'paper' with dropout are incorrect"
