...
class TestFibiNET:
    ...
    def sample_data(self):
        # Generate sample data for FiBiNET model
        data = []
        for i in range(100):
            data.append({
    ...
    def test_initialization(self, model):
        assert model.name == "FiBiNET"
        assert model.embed_dim == 16
        assert model.bilinear_type == 'field_interaction'
    ...
    def test_save_load(self, model, sample_data, tmp_path):
        model.fit(sample_data)
        save_path = tmp_path / "fibinet_model.pt"
        model.save(str(save_path))
        
        loaded_model = Fibinet_base.load(str(save_path))
