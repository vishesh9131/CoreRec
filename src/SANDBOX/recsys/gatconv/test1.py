...
class MovieGATRecommender(nn.Module):
    def __init__(self, num_users, num_movies, hidden_dim, out_dim, heads=2, dropout=0.1):
        super(MovieGATRecommender, self).__init__()
...
model = MovieGATRecommender(num_users, num_movies, hidden_dim=64, out_dim=32)
...
for movie_id in recommended_movies:
    ...
    if movie_id in movie_id_to_title:
        recommended_titles.append(movie_id_to_title[movie_id])
    else:
        print(f"Warning: Movie ID {movie_id} not found in movie_id_to_title dictionary.")
