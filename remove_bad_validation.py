...
for fpath in files:
    try:
        ...
        # Remove lines with validation that appear before function signature closes
        new_lines = []
        i = 0
        while i < len(lines):
            line = lines[i]
            
            # If we see "# Validate" before a closing paren with ->, skip it and next validate line
            if '# Validate' in line:
                # Check if we're in a function signature (look ahead for closing paren)
                in_signature = False
                ...
                if in_signature:
                    print(f"  Removing bad validation from {fpath} line {i+1}")
                    i += 1  # Skip # Validate line
                    # Skip validate_xxx line
                    while i < len(lines) and ('validate_' in lines[i] or lines[i].strip() == ''):
                        i += 1
                    continue
