...
from corerec.engines.unionizedFilterEngine.nn_base.BST_base import (
    BST_base, BehaviorSequenceTransformer as BSTModel, MultiHeadAttention, 
    TransformerBlock, TokenEmbedding, PositionalEmbedding, HookManager, FeatureEmbedding
)
...
class PatchedBST_base(BST_base):
    ...
    def fit(self, interactions, user_ids=None, item_ids=None, item_features=None):
        ...
        if item_ids is not None:
            ...
            self.iid_map = {iid: i + 1 for i, iid in enumerate(self._BaseCorerec__item_ids)}  # +1 for padding
...
class TestBSTComponents(unittest.TestCase):
    ...
    def test_feature_embedding(self):
        ...
        embeddings = nn.ModuleList([
            nn.Embedding(dim, embed_dim) for dim in self.field_dims
        ])
    ...
    class SimpleBSTModel(nn.Module):
        def __init__(self, vocab_size, hidden_dim, field_dims, seq_len, num_heads, num_layers):
            ...
            self.feature_proj = nn.Linear(hidden_dim // len(field_dims) * len(field_dims), hidden_dim)
        ...
        def forward(self, seq, item, features):
            ...
            attention_weights = [torch.rand(batch_size, self.hidden_dim // 8, seq.size(1), seq.size(1))]
